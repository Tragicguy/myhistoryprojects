...
STDMETHODIMP DuiLib::CWebBrowserUI::GetTypeInfoCount( UINT *pctinfo )
{
	if (m_pWebBrowserEventHandler)
	{
		return m_pWebBrowserEventHandler->GetTypeInfoCount(pctinfo);
	}
	*pctinfo = 0;
	return E_NOTIMPL;
}
...
STDMETHODIMP DuiLib::CWebBrowserUI::GetTypeInfo( UINT iTInfo, LCID lcid, ITypeInfo **ppTInfo )
{
	if (m_pWebBrowserEventHandler)
	{
		return m_pWebBrowserEventHandler->GetTypeInfo(iTInfo, lcid, ppTInfo);
	}
	return E_NOTIMPL;
}
...
STDMETHODIMP DuiLib::CWebBrowserUI::GetIDsOfNames( REFIID riid, OLECHAR **rgszNames, UINT cNames, LCID lcid,DISPID *rgDispId )
{
	if (m_pWebBrowserEventHandler)
	{
		return m_pWebBrowserEventHandler->GetIDsOfNames(riid, rgszNames, cNames, lcid, rgDispId);
	}
	return E_NOTIMPL;
}
	...
	default:
		{
			if (m_pWebBrowserEventHandler)
			{
				return m_pWebBrowserEventHandler->Invoke(dispIdMember, riid, lcid, wFlags, pDispParams, pVarResult, pExcepInfo, puArgErr);
			}
...
void DuiLib::CWebBrowserUI::GetLocationUrl(BSTR* bstrStr)
{
	if (m_pWebBrowser2)
		m_pWebBrowser2->get_LocationURL(bstrStr);
}
...
STDMETHODIMP DuiLib::CWebBrowserUI::GetExternal( IDispatch** ppDispatch )
{
	if (m_pWebBrowserEventHandler)
	{
		return m_pWebBrowserEventHandler->GetExternal(ppDispatch, this);
	}
	return S_FALSE;
}
