...
namespace DuiLib
{
	...
	{
		Init(_ParentNode, false);
	}
	
	void CTreeNodeUI::Init(CTreeNodeUI* _ParentNode, bool bLeaf)
	{
		...
		pItemButton->SetAttribute(_T("padding"), _T("3, 4, 0, 0"));
	...
	CTreeNodeUI::CTreeNodeUI(bool bLeaf, CTreeNodeUI* _ParentNode)
	{
		Init(_ParentNode, bLeaf);
	}
	...
	void CTreeNodeUI::DoEvent( TEventUI& event )
	{
		...
		{
			CControlUI* pControlModify = m_pManager->FindControl("treemenu");			
			pControlModify->SetVisible(false);
			pControlModify = m_pManager->FindControl("modifyoradd");
			pControlModify->SetVisible(false);
		...
		else if (UIEVENT_CONTEXTMENU == event.Type)
		{
			CControlUI* pControlModify = m_pManager->FindControl("modifyoradd");
			pControlModify->SetVisible(false);

			CTreeNodeUI* pNode = GetFirstCTreeNodeUIFromPoint(event.ptMouse);
			pTreeView->m_pNodeModifyOrAdd = pNode;
			
			pControlModify = m_pManager->FindControl("treemenu");
			RECT rt;
			rt.left = event.ptMouse.x + 1;
			rt.top = event.ptMouse.y + 1;
			rt.right = rt.left + 80;
			rt.bottom = rt.top + 40;			
			if (pNode->IsLeaf())
			{
			...
			pControlModify->SetPos(rt);
			pControlModify->SetVisible(true);
		}
	}

	void CTreeNodeUI::SetExpand(bool bExpand)
	{
		GetFolderButton()->SetCheck(!bExpand);
	}

	void CTreeNodeUI::SetIsLeaf(bool bLeaf)
	{
		m_bLeaf = bLeaf;
		if (bLeaf)
		{
			SetAttribute(_T("folderattr"),_T("padding=\"0,1,0,0\" width=\"1\" height=\"1\""));
			GetItemButton()->SetAttribute(_T("padding"), "20,4,0,0");
		}
		SetVisibleFolderBtn(!bLeaf);
	}
...
// 			return -1;
		...
		if(!pParent->IsVisible() || pParent->GetFolderButton()->GetCheck())
		{
	...
	void CTreeViewUI::EndDrag(CTreeNodeUI* dstParent)
	{
		...
		m_pDragingCtrl->SetPos(rt);
		if (m_pNodeNeedMove != NULL && dstParent != NULL && m_pNodeNeedMove != dstParent && !dstParent->IsLeaf())
		{
	...
	{	
		...
		CTreeNodeUI* pOldParent = pNode->GetParentNode();
		...
		m_pManager->SendNotify(this, "TreeNodeMove", (WPARAM)pOldParent, (LPARAM)pNode);
	}
