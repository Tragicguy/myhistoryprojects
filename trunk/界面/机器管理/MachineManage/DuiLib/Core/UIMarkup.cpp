...
CMarkupNode::CMarkupNode(CDuiMarkup* pOwner, int iPos) : m_pOwner(pOwner), m_iPos(iPos), m_nAttributes(0)
{
}
...
CDuiMarkup::CDuiMarkup(LPCTSTR pstrXML)
{
    m_pstrXML = NULL;
    m_pElements = NULL;
    m_nElements = 0;
...
void CDuiMarkup::SetPreserveWhitespace(bool bPreserve)
{
...
bool CDuiMarkup::LoadFromMem(BYTE* pByte, DWORD dwSize, int encoding)
{
#ifdef _UNICODE
...
bool CDuiMarkup::LoadFromFile(LPCTSTR pstrFilename, int encoding)
{
    Release();
...
CMarkupNode CDuiMarkup::GetRoot()
{
...
bool CDuiMarkup::_ParseData(LPTSTR& pstrText, LPTSTR& pstrDest, char cEnd)
{
...
bool CDuiMarkup::_Failed(LPCTSTR pstrError, LPCTSTR pstrLocation)
{
    // Register last error
