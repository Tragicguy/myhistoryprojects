...
BOOL CMenuWnd::Receive(ContextMenuParam param)
{
	...
	case 1:
		{
			Close();
			if (param.lParam != NULL && m_pMenuEventNotice != NULL)
			{
				m_pMenuEventNotice->SetUserData((LPCTSTR)param.lParam);
				::PostMessage(m_hParent, WM_MENU, NULL, (LPARAM)m_pMenuEventNotice);
			}
		}
...
void CMenuWnd::Init(CMenuElementUI* pOwner, STRINGorID xml, LPCTSTR pSkinType, POINT point, CControlUI* pMenuEventNotice)
{
	m_BasedPoint = point;
    m_pOwner = pOwner;
    m_pLayout = NULL;
	m_pMenuEventNotice  = pMenuEventNotice;
	if (pSkinType != NULL)
		m_sType = pSkinType;
