...
#define BMP_WIDTH   1366          //位图宽度
#define BMP_HEIGHT  768          //位图高度
...
void CSpecialFXDlg::OnTimer(UINT nIDEvent) 
{
	...
	if(nIDEvent == 2)
	{
		...
		CPoint point;
		point.x = rcPicture.left / 2 + rcPicture.right / 2;
		point.y = rcPicture.top / 2 + rcPicture.bottom / 2 - 25;
		

		m_myWater.WarpBlob(point.x-15,point.y, 20,1000000,m_myWater.m_iHpage);
	}
...
{
	m_nTimer = SetTimer(TIMER_START,20,NULL);
