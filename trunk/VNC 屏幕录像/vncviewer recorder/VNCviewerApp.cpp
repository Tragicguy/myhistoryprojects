...
VNCviewerApp::VNCviewerApp(HINSTANCE hInstance, LPTSTR szCmdLine) {
	...
	char* pdest = strstr(szCmdLine, "-");
	if (pdest != NULL)
	{
		char szCmdLineA[MAX_PATH] = {0};
		char szCmdLineB[MAX_PATH] = {0};

		memcpy(szCmdLineA, szCmdLine, pdest - szCmdLine);
		memcpy(szCmdLineB, pdest + 1, lstrlen(szCmdLine) - (pdest - szCmdLine + 1));

		//MessageBox(NULL, szCmdLineA, "1", 0);
		//MessageBox(NULL, szCmdLineB, "2", 0);

		m_options.SetFromCommandLine(szCmdLineA);
		pdest = strstr(szCmdLineB, "-");
		if (pdest)
		{
			char szCmdLineC[MAX_PATH] = {0};
			char szCmdLineD[MAX_PATH] = {0};
			memcpy(szCmdLineC, szCmdLineB, pdest - szCmdLineB);
			memcpy(szCmdLineD, pdest + 1, lstrlen(szCmdLineB) - (pdest - szCmdLineB + 1));
			gl_hWnd = (HWND)(int)atoi(szCmdLineC);
			gl_iRescFlag = atoi(szCmdLineD);
		}
		else
		{
			gl_hWnd = (HWND)(int)atoi(szCmdLineB);
		}		
	}
	else
	{
	...
	// Logging info
	vnclog.SetLevel(4);
	vnclog.Print(3, "%s %d\n",szCmdLine,GetModuleHandle(NULL));
