...
#define VWR_WND_CLASS_NAME _T("RTSServer")

#define VNC_TITLE_RESFALSG _T("Web1800-远程救援窗口")
...
//#define VNC_RECORD_PLAYER
#define VNC_RECORD_DATA_FILE "c:\\record.vncdata"
#define VNC_RECORD_HEAD_FILE "c:\\record.vncheader"

fstream gl_fStreamHeader(VNC_RECORD_HEAD_FILE, ios::in | ios::out | ios::binary | ios::app);
fstream gl_fStream(VNC_RECORD_DATA_FILE, ios::in | ios::out | ios::binary | ios::app);

unsigned long gl_ulFileOffset = 0;

extern HWND gl_hWnd;
extern int gl_iRescFlag;
extern HWND gl_hWndSelf;
...
void ClientConnection::Init(VNCviewerApp *pApp)
{
	...
	//Added by YCW to read parameters from Config files

	m_bExitRTSServer = true;
	m_ProgramConfig.SetConfigFileName("RTSServer.ini");
	//The following is the new source code added by xpb 2002/02/26
	m_iQuitTimerInterval = m_ProgramConfig.ReadIntConfigParameter("RTS Options With Int Type", "ServerIdleTimeout", 300);
	m_iQuitTimerInterval = 1000 * m_iQuitTimerInterval;
	//Added by YCW on 2005-1-26 for multiple language
	m_ProgramConfig.ReadUIString("SupporterMessageBoxTitle", m_sMessTitle, "ShowMatic Server");
	// We take the initial conn options from the application defaults
	m_opts = m_pApp->m_options;
	//Added by YCW on 2005-1-27 to Read the ViewOnly and DsiableClipboard from Config file
	m_opts.m_ViewOnly=m_ProgramConfig.ReadIntConfigParameter("RTS Options With Int Type", "viewonly",0)!= 0; 
	m_opts.m_DisableClipboard=m_ProgramConfig.ReadIntConfigParameter("RTS Options With Int Type", "disableclipboard",0)!= 0;  
	if (gl_iRescFlag)
	{
		m_opts.m_ViewOnly = 0;
	}
...
void ClientConnection::Run()
{
	...
	vnclog.Print(1, _T("EnableFullControlOptions return.\n"));

	CreateLocalFramebuffer();
	vnclog.Print(1, _T("CreateLocalFramebuffer return.\n"));
	
	SetupPixelFormat();
	vnclog.Print(1, _T("SetupPixelFormat return.\n"));

	SetFormatAndEncodings();
	vnclog.Print(1, _T("SetFormatAndEncodings return.\n"));
	if(m_iQuitTimerInterval)
	{
		m_QuitTimer = SetTimer(m_hwnd, IDT_TIMERQUIT, m_iQuitTimerInterval, (TIMERPROC) NULL);
	}

	// This starts the worker thread.
	// The rest of the processing continues in run_undetached.
	start_undetached();
	vnclog.Print(1, _T("start_undetached return.\n"));
}
...
void ClientConnection::CreateDisplay() 
{
	...
	wndclass.hInstance		= m_pApp->m_instance;
	wndclass.hIcon			= NULL;
	//add by shaojun at 2011-04-12 
	if (gl_iRescFlag)
	{		
		m_pApp->m_options.m_localCursor = NOCURSOR;
	}
	...
	//Added by YCW on 20050510: Write this wnd to the config file
	//m_ProgramConfig.WriteIntConfigParameter("RTS Options With Int Type","VNCWindowHandle",(int)m_hwnd1);
	//modify by shaojun at 2010-09-15 post the hwnd handle to 1800 process
	::PostMessage(gl_hWnd,WM_NOTIECE_VNCHANDLE,(WPARAM)m_hwnd1,0);
	gl_hWndSelf = m_hwnd1;
	...
	if (!m_opts.m_restricted) {
		...
		AppendMenu(hsysmenu, MF_SEPARATOR, NULL, NULL);
         */
		//if(!m_opts.m_ViewOnly) //Added by YCW on 20050419: Only active in Non-ViewOnly mode
		{
			AppendMenu(hsysmenu, MF_STRING, ID_CONN_CTLALTDEL,	_T("Send Ctl-Alt-Del")); 
			//AppendMenu(hsysmenu, MF_STRING, ID_LOCK_MOUSE,   _T("&Lock\\Unlock Mouse\tCtrl-G"));
...
//This section is updated by xpb 2002/02/06, begin:
	//The following original source code is disabled by xpb 2002/02/06.
	//The following is the new source code added by xpb 2002/02/06
		//m_opts.LoadUserPrefsFromCFGFile(&m_ProgramConfig);
//This section is updated by xpb 2002/02/06, end.
...
void ClientConnection::EnableFullControlOptions()
{
	if (m_opts.m_ViewOnly) {
		SwitchOffKey();
		vnclog.Print(1, _T("SwitchOffKey return.\n"));
...
void ClientConnection::SwitchOffKey()
{
	...
	*/
	...
	vnclog.Print(1, _T("XK_Alt_L return.\n"));
...
bool ClientConnection::AuthenticateVNC(char *errBuf, int errBufSize, bool *again)
{
	...
	//The following is the new source code added by xpb 2002/02/06
	char hcpasswd[MAXPWLEN] = "";
	m_ProgramConfig.ReadStringConfigParameter("RTS Options With String Type", "Password",hcpasswd, "RTS");

	if(strcmp(hcpasswd, "*") == 0) //use the password which stored in RTSServer.password file
	{
		char lpSysDirBuffer[_MAX_PATH];
		GetSystemDirectory(lpSysDirBuffer, _MAX_PATH);
		strcat(lpSysDirBuffer, "\\RTSServer.password");

		ProgramConfig tmpProgramConfig;

		tmpProgramConfig.SetConfigFileName(lpSysDirBuffer);
		tmpProgramConfig.ReadStringConfigParameter("Password", "Password",hcpasswd, "RTS");
	}

	vncEncryptPasswd(m_encPasswd, hcpasswd);
	//This section is updated by xpb 2002/02/06, end.
...
void ClientConnection::ReadServerInit()
{
	...
	{
		SetWindowText(m_hwnd1, VNC_TITLE_RESFALSG);
		vnclog.Print(0, _T("Desktop name \"%s\"\n"),VNC_TITLE_RESFALSG);
	}	
	...
	SizeWindow(true);
	vnclog.Print(1, _T("SizeWindow return.\n"));
	//Added by YCW: make it on TopMost	---Begin
	//SetWindowPos(m_hwnd1,HWND_TOPMOST, 0,0,0,0, SWP_NOMOVE|SWP_NOSIZE); 
	//SetForegroundWindow(m_hwnd1);
	//Added by YCW: make it on TopMost	---End
}
...
void ClientConnection::ReadInteractionCaps()
{
	...
	vnclog.Print(1, _T("Interaction capability list header received.\n"));
	...
	vnclog.Print(1, _T("Interaction capability list received.\n"));
}
...
void ClientConnection::CreateLocalFramebuffer() {
	...
	//Multiple language support Added on 2005-2-25 by  YCW
	// char winTitle[64];

	char strInitialMSG[256];

	m_ProgramConfig.SetConfigFileName("RTSServer.ini");

	//m_ProgramConfig.ReadStringConfigParameter(m_sUISectionName, "WindowTitle",winTitle, "ShowMatic");

	m_ProgramConfig.ReadUIString("ScreenInitialMSG",strInitialMSG, "Please wait - initial screen loading!");
	/*
		DrawText (m_hBitmapDC, _T("Please wait - initial screen loading"), -1, &rect,
		*/
	DrawText (m_hBitmapDC, strInitialMSG, -1, &rect,
...
{	// This is a static method, so we don't know which instantiation we're 
	...
	switch (iMsg) {
	case WM_TIMER:
//This section is updated by xpb 2002/11/01, begin:
	  if (wParam == _this ->m_QuitTimer) //the idle timeout
	  {

			KillTimer(_this ->m_hwnd, IDT_TIMERQUIT);
			//When idle timeout it will send message to inform
			//the RTSServer.exe to exit
			// char winTitle[64];

			char strIdleMSG[256];

			_this ->m_ProgramConfig.SetConfigFileName("RTSServer.ini");

			int isListeningMode = _this ->m_ProgramConfig.ReadIntConfigParameter("RTS Tunnel Options", "IsListeningMode", 1 );

			// _this ->m_ProgramConfig.ReadStringConfigParameter(_this->m_ProgramConfig.m_sUISectionName, "WindowTitle",winTitle, "ShowMatic");

			_this ->m_ProgramConfig.ReadUIString("IdleTimeoutMSG",strIdleMSG, "Idle timeout,the system will terminate!");

			///////////////////////////////////////////////

			// HWND phWnd = ::FindWindow(NULL,winTitle); 
			//HWND phWnd = (HWND)_this ->m_ProgramConfig.ReadIntConfigParameter ("RTS Options With Int Type", "Wrapper Window Handle", 0);


			//if(phWnd) 
			//{
				// MessageBox(NULL,strIdleMSG,winTitle,MB_OK);
				::PostMessage(gl_hWnd, MYWM_IDLE_TIMEOUT, 0, 0); 
			//}

			_this->KillThread();
			DestroyWindow(hwnd);
	  }
//This section is updated by xpb 2002/02/26, end.
	  return 0;
					   ...
					   WPARAM wParam, LPARAM lParam) 
{
	...
	{		
		...
		case ID_FULLSCREEN:
			...
			break;
		case ID_LOCK_MOUSE:
			TTStr->lpszText = "Lock\\Unlock Mouse";
			break;
	...
	case WM_SYSCOMMAND:
		...
		case ID_NEWCONN:
			...
			return 0;
		case ID_DISCONNECT:
			//MessageBox(NULL,"ID_DISCONNECT","ID_DISCONNECT",0);
		...
		case ID_FULLSCREEN:
			...
			return 0;
		case ID_LOCK_MOUSE:
			//如果，是非控制模式或者非救援光盘，直接返回
			if (_this->m_opts.m_ViewOnly == true || gl_iRescFlag == 0)
			{
				return 0;
			}
			// Lock mouse
			...
			{
				RECT rc,rc1;
				::GetWindowRect(_this->m_hwnd1, &rc);
				::GetClientRect(_this->m_hwnd1, &rc1);
				/*
				if (GetMenuState(GetSystemMenu(_this->m_hwnd1, FALSE),ID_TOOLBAR,MF_BYCOMMAND) == MF_CHECKED)
				{
					rc.top = rc.bottom - rc1.bottom + rc.bottom - rc.top - rc1.bottom;
				}
				else
				{
					rc.top = rc.bottom - rc1.bottom;
				}*/
				rc.top = rc.bottom - rc1.bottom;
				//rc.top = rc.top + 10;
				...
				rc.left = rc.left + 5;
				rc.right = rc.right - 5;
				//::ClientToScreen(_this->m_hwnd1, lp);
				::ClipCursor(&rc);
				gCnt++;
				::PostMessage(gl_hWnd,WM_NOTICE_VNCSTATUS,1,0);
			}
			else
			{
				::PostMessage(gl_hWnd,WM_NOTICE_VNCSTATUS,0,0);
				::ClipCursor(NULL);
				gCnt--;
			}			
			return 0;
...
//				SendMessage(_this->m_hToolbar, TB_SETSTATE, (WPARAM)ID_CONN_ALTDOWN,
	...
	case WM_ACTIVATE:
		{
			if ( _this->m_opts.m_ViewOnly ==false && gl_iRescFlag == 1 )
			{
				if ( LOWORD(wParam) == WA_INACTIVE )
				{
					gCnt = 1;
					::PostMessage(hwnd,WM_COMMAND,ID_LOCK_MOUSE,0);
				}				
			}
			break;
		}
	case MY_NO_VIEWONLY: //Added by YCW on 20050510 to Change from ViewOnly mode to Control mode
		{
			vnclog.Print(0, _T("Control Mode Message Received\n"));
			_this->m_opts.m_ViewOnly=false; 
			_this->EnableFullControlOptions();
			return 0;
		}

	case MY_BACK_TO_VIEWONLY : //Added by YCW on 200510524 to Change from Control mode to ViewOnly mode
		{
	...
	case MY_CONNECTION_PROFILE:// Added by YCW on 20060106 to change the Connection profile
		{
			int nProfile=0;
			int nLevel=0;
			int nScale=0;
			...
			nProfile=(int)wParam/10;
			nLevel=(int)wParam%10;
			nScale=(int)lParam;

			for (int i = rfbEncodingRaw; i <= LASTENCODING; i++)
				_this->m_opts.m_UseEnc[i] = true;

			_this->m_opts.m_UseEnc[3] = false;	
			
			if(nProfile==0) // Low bandwidth
			{

				_this->m_opts.m_PreferredEncoding = rfbEncodingTight;
				_this->m_opts.m_useCompressLevel = true;
				_this->m_opts.m_compressLevel = nLevel;
				_this->m_opts.m_Use8Bit = true;
			}
			else if(nProfile==1) //Default
			{
				_this->m_opts.m_PreferredEncoding = rfbEncodingTight;
				_this->m_opts.m_useCompressLevel = false;
				_this->m_opts.m_enableJpegCompression = true;
				_this->m_opts.m_jpegQualityLevel =nLevel;
				_this->m_opts.m_Use8Bit = false;
			}
			else if(nProfile==2) //High speed
			{
				_this->m_opts.m_PreferredEncoding = rfbEncodingHextile;
				_this->m_opts.m_Use8Bit = false;
			}

			

			/* Add by YCW on 2006-6-29 for Resolution */
			...
			if (nScale > 0) {
				_this->m_opts.m_scale_num = nScale;
				_this->m_opts.m_scale_den = 100;
				_this->m_opts.FixScaling();
				_this->m_opts.m_FitWindow = false;
				_this->m_opts.m_scaling = (_this->m_opts.m_scale_num > 1) || (_this->m_opts.m_scale_den > 1);
			} else {

				if (nScale==0) { //"Auto"
					_this->m_opts.m_FitWindow = true;
					_this->m_opts.m_scaling = true;
				} else {
					_this->m_opts.m_FitWindow = false;
				}
	...
	case WM_CLOSE:
		{
			...
			char strTitleMSG[256];
			char strTextMSG[256];

			_this ->m_ProgramConfig.SetConfigFileName("RTSServer.ini");


			_this ->m_ProgramConfig.ReadUIString("ConfirmationCaption",strTitleMSG, "Confirmation dialog");

			_this ->m_ProgramConfig.ReadUIString("ConfirmationText",strTextMSG, "Are you sure to exit this window?");


			//Added by YCW on 2007-4-12. Popup confirmation dialog. and change all the PostMessage(m_hwnd1, WM_CLOSE, 0, 0) for exceptions to PostMessage(m_hwnd1, WM_CLOSE, 2, 0);;
			if(wParam!=2 && wParam!=1)
				if(MessageBox(NULL,strTextMSG,strTitleMSG,MB_OKCANCEL)==IDCANCEL)
					return 0;


			//Added by xpb 2002/09/19
			//When RTSTransaction is closed by the user, it will send message to inform
			//the RTSServer.exe to exit
			if(wParam!=1)
			{
				/*
				...
				strcpy(theFileName, strCurrentPath + 1);
				strcat(theFileName, "RTSServer.Config");
				*/
				// char winTitle[64];
				//_this ->m_ProgramConfig.SetConfigFileName("RTSServer.ini");
				...
				if(isListeningMode)
				{
					if(_this ->m_bExitRTSServer)
					{
						...
						//if(phWnd) 
						//{
							::PostMessage(gl_hWnd, MYWM_EXIT, 0, 0); 
						//}
					}
				}
				else
...
{	
	...
	case WM_LBUTTONDOWN:
		{
			gCnt = 0;
			::PostMessage(gl_hWndSelf,WM_COMMAND,ID_LOCK_MOUSE,0);
		}
	case WM_LBUTTONUP:
	case WM_MBUTTONDOWN:
	case WM_MBUTTONUP:
		...
		if (m_waitingOnEmulateTimer) {
			...
			if ( (msg == WM_LBUTTONDOWN && !(keyflags & MK_RBUTTON))
				...
				if (!m_emulate3ButtonsTimer) {
					...
					//MessageBox(NULL,"m_emulate3ButtonsTimer","m_emulate3ButtonsTimer",0);
					//PostMessage(m_hwnd1, WM_CLOSE, 0, 0);
					PostMessage(m_hwnd1, WM_CLOSE, 2, 0);
					return;
				}
...
void* ClientConnection::run_undetached(void* arg) {
	...
	vnclog.Print(3, _T("Update-processing thread started\n"));
	...
	try {
...
#ifdef VNC_RECORD_PLAYER
			int bytes = 1;
			gl_fStreamHeader.read((char *) &msgType, 1);
#else
			int bytes = recv(m_sock, (char *) &msgType, 1, MSG_PEEK);
			ofstream fOut(VNC_RECORD_HEAD_FILE, ios::in | ios::out | ios::binary | ios::app);
			fOut.write((char *) &msgType, sizeof(CARD8));
			fOut.close();
#endif // VNC_RECORD_PLAYER


			  //记录此处的值，并且如果不是第一次读取，则将读取的时间差记录
		/*  if (!bFirstLoop)
			  {
				  unsigned long lInterval = ::GetTickCount() - lLastTime;
				  fOut.write((char*)&lInterval, sizeof(lInterval));
				  lLastTime = ::GetTickCount();
			  }
			  else
			  {
				  lLastTime = ::GetTickCount();
			...
			case rfbServerCutText:
				ReadServerCutText();
				break;
						case rfbFullScreenMode:
				rfbFullScreenModeMsg mfullscnmsg;
				ReadExact((char *)&mfullscnmsg, sz_rfbFullScreenModeMsg);

				char strInfo[128];

				m_ProgramConfig.ReadUIString("SupporterFullScreenMode", strInfo, "Full screen DOS window is activated on client machine. You can not see this DOS window");
				{
				...
				MessageBox(NULL, strInfo,m_sMessTitle,MB_OK);
		...
		m_running = false;
		//MessageBox(NULL,"WarningException","WarningException",0);
...
void ClientConnection::ReadExact(char *inbuf, int wanted)
{
	if (m_sock == INVALID_SOCKET && m_bKillThread)
		throw QuietException("ClientConnection::ReadExact Invalid Socket,Connection closed.");
	...
	while (wanted > 0) {
		...
		//gl_fStream.seekg()
		gl_fStream.read(inbuf+offset, wanted);
		int bytes = offset = wanted;
#else
		int bytes = recv(m_sock, inbuf+offset, wanted, 0);
#endif		
...
#ifndef VNC_RECORD_PLAYER
	fstream fStream(VNC_RECORD_DATA_FILE, ios::in | ios::out | ios::binary | ios::app);
	fStream.write(inbuf, offset);
	fStream.close();	
#endif
	gl_ulFileOffset += offset;
}
