...
Log exceptionlog(Log::ToFile, 5, "data\\RTSServerTransactionExc.dat", true);

bool bSetLogLevel = false;

Exception::Exception(const char *info)
{
	assert(info != NULL);
	m_info = new char[strlen(info)+1];
	memset(m_info, 0, strlen(info)+1);
	strcpy(m_info, info);

	if (!bSetLogLevel)
	{
		bSetLogLevel = true;
		exceptionlog.SetLevel(4);
	}
}
...
void QuietException::Report()
{
	...
	exceptionlog.Print(0, VNCLOG("Warning : %s\n"), m_info);
}
...
void WarningException::Report()
{
	...
	_RPT1(_CRT_WARN, "Warning : %s\n", m_info);
#endif
//	Disabled by YCW, for avoiding this message be displayed circularly
//MessageBox(NULL, m_info, " info", MB_OK| MB_ICONEXCLAMATION | MB_SETFOREGROUND | MB_TOPMOST);
	exceptionlog.Print(0,"Warning : %s\n", m_info);
}
...
void ErrorException::Report()
{
	...
	_RPT1(_CRT_WARN, "Warning : %s\n", m_info);
#endif
	MessageBox(NULL, m_info, "RTSClient info", MB_OK | MB_ICONSTOP | MB_SETFOREGROUND | MB_TOPMOST);
	exceptionlog.Print(0,"Warning : %s\n", m_info);
}
...
void AuthException::Report()
{
	...
	_RPT1(_CRT_WARN, "Warning : %s\n", m_info);
#endif
	MessageBox(NULL, m_info, "RTSClient authentication info", MB_OK| MB_ICONEXCLAMATION | MB_SETFOREGROUND | MB_TOPMOST);
	exceptionlog.Print(0,"Warning : %s\n", m_info);
}
