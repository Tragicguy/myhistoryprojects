...
{		
		...
		while(!g_ExitFlag)
		{
			...
			hwnd = gl_find.GetChildWindow(hwnd, 8);
			if (!hwnd)
			{
				Sleep(300);
				continue;
			}			
			HWND hwndAcc = gl_find.GetChildWindow(hwnd, 3);
			HWND hwndNick = gl_find.GetChildWindow(hwnd, 4);
			HWND hwndPwd = gl_find.GetChildWindow(hwnd, 5);
			HWND hwndPwdConfirm = gl_find.GetChildWindow(hwnd, 6);
			HWND hwndIdentity = gl_find.GetChildWindow(hwnd, 7);
			HWND hwndIntro = gl_find.GetChildWindow(hwnd, 9);
			gl_find.SetEditText(hwndAcc, rgPack.strAcc);
			gl_find.SetEditText(hwndNick, rgPack.strNickName);
			gl_find.SetEditText(hwndPwd, rgPack.strPwd);
			gl_find.SetEditText(hwndPwdConfirm, rgPack.strPwd);
			gl_find.SetEditText(hwndIdentity, rgPack.strIdentity);
			gl_find.SetEditText(hwndIntro, rgPack.strIntroMan);
			rgPack.iStatus = 1;
			rgPack.strMemo = "填写完成";
