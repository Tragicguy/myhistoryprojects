...
CArray<ProxyStru,ProxyStru> gl_ProxyStruList;
int gl_ProxyLen = 0;
/************************************************************************/
/*                                                                      */
/************************************************************************/
...
C51JobWebPost::C51JobWebPost()
{
	...
	m_strHeader += "\r\nUser-Agent:Mozilla/4.0 (compatible; MSIE 6.0; Windows NT 5.2; SV1; DMBLed; Foxy/2; EmbeddedWB 14.52 from: http://www.bsalsa.com/ EmbeddedWB 14.52; .NET CLR 1.1.4322; .NET CLR 3.0.04506.648; .NET CLR 3.5.21022; .NET CLR 2.0.50727; .NET4.0C; .NET CLR 3.0.4506.2152; .NET CLR 3.5.30729)";
...
bool C51JobWebPost::UserRegister()
{
	...
	{		
		...
		int iPort = 0;
		/*
		if (::GetTickCount() - gl_lastUserOwnIp > 2 * 60 * 1000)
		{
			//gl_lastUserOwnIp = ::GetTickCount();
			gl_lastUserOwnIp =  - 1000;
			bUserProxy = FALSE;
			ShowMessage("使用本机IP注册");
		}
		else
		{	
			bUserProxy = TRUE;
			gl_iLoopCount++;
			if (gl_iLoopCount > gl_ProxyLen -1)
			{
				gl_iLoopCount = 0;
			}
			if (gl_ProxyLen < 1)
			...
			ProxyStru pro;
			pro = gl_ProxyStruList.GetAt(gl_iLoopCount);
			strIP = pro.strIp;
			iPort = pro.iPort;
			CString strTestShow = "";
			strTestShow.Format("代理信息：%d/%d ，当前使用IP：%s",gl_iLoopCount+1,gl_ProxyLen,strIP);
			ShowMessage(strTestShow);
		}
		*/
		//取IP
		//action=userreg&uname=juntest&upwd=wawawo20&confirm_upwd=wawawo20&srcname=kslkdkd&boy=1&fullname=%u9648%u793C&zhengjian=430225198110100016&mophone=15019212356&spread=&ucode=%u9700%u513F%u611F%u4EF7&twopass=&confirm_twopass=&email=&protocol=on
		m_strServerURL = "passport.kedou.com";
		CString strError = "";
		CString sendStr = "";
		ShowMessage("正在获取注册页面...");
		CString objectlink = "";
		
		objectlink = "/front/noLogin/goRegist2_front.htm?userType=1&returnCode=quick&site_id=1024_gameclient&return_url=http://www.kedou.com";
		...
		while(!g_ExitFlag)
		{
			try
			{
				...
				iRet = RequestGetPageExExEx(m_strServerURL,objectlink,strHeaders,
					strError,0,bUserProxy,0,strIP,iPort,"","");
				if (m_strWorkBuf.Find("蝌蚪通行证-注册") < 0)
				{
					m_nErrorTime ++;
					DeleteObjectExEx();
				...
				if (m_iCheckFlag == 0)
				{
					...
					strHeaders = m_strHeader + "\r\nReferer:http://passport.kedou.com/front/noLogin/goRegist2_front.htm?userType=1&returnCode=quick&site_id=1024_gameclient&return_url=http://www.kedou.com";
					objectlink.Format("/checkCode.htm");
					iRet = RequestGetPageExExEx(m_strServerURL,objectlink,strHeaders,
						strError,1,bUserProxy,0,strIP,iPort,"","");
					//objectlink.Format("/checkCode.htm?checkCode2=checkCode2",strTime);
					//iRet = RequestGetPageExExEx(m_strServerURL,objectlink,strHeaders,
					//	strError,1,bUserProxy,0,strIP,iPort,"","");
					//
					if (iRet != 0)
					{
						if (1)
						{
							if (iRet == -111 || iRet == -4)//代理有问题
							{
								rgPack.strMemo = "网络错误";
								ShowMessage("网络错误");
// 								gl_ProxyStruList.RemoveAt(gl_iLoopCount);
...
// 								gl_iLoopCount--;
				...
				//rgPack.strPwd = "wawawo20";
				string strPwd = MD5Encode((LPTSTR)(LPCTSTR)rgPack.strPwd);
				postStr.Format("member.memberName=%s", rgPack.strAcc);
				objectlink = "/front/noLogin/chkUserName.htm";
				iRet = RequestPostPageExExEx(m_strServerURL,objectlink,strHeaders,postStr,strError,0,bUserProxy,0,strIP,iPort,"","");
				if (iRet < 0)
				{
					return false;
				}
				if (m_strWorkBuf.Find("flag:false") < 0)
				{
					ShowMessage("用户名非法");
					return false;
				}
				...
				strHeaders += "\r\nContent-Type: application/x-www-form-urlencoded";
				strHeaders += "\r\nCache-Control:no-cache";
				objectlink = "/front/noLogin/reg.htm";
				CString strDataFront = "return_url=http%3A%2F%2Fwww.kedou.com&site_id=1024_gameclient&token=&";
				postStr.Format("member.memberPwd=%s&member.linkEmail=&member.linkMobile=&nextCode=site&returnCode=quick&return_code=&userType=1&regVersion=&regEnvironment=&checkCodeError=&member.memberName=%s&member.realName=&member.idCardNo=&member.companyName=&member.linkMan=&checkCode=%s",
					strPwd.c_str(),
					rgPack.strAcc,
					strCheckCode);
				postStr = strDataFront + postStr;
				iRet = RequestPostPageExExEx(m_strServerURL,objectlink,strHeaders,postStr,strError,0,bUserProxy,0,strIP,iPort,"","");//
...
// 							gl_iLoopCount--;
				...
				int iPos = m_strWorkBuf.Find("恭喜您已经注册成功");
				if ( iPos >= 0)
				{
                    ShowMessage("注册成功");
					rgPack.iStatus = 1;
					...
					if (bUserProxy)
					{
						strShow.Format("nn %s %d ",strIP,iPort);
						WriteLog(strShow,3);
					}
				...
				{//注册失败						
					...
					rgPack.iStatus = 2;
					if (m_strWorkBuf.Find(">验证码不正确</em>") > 0)
					{
						rgPack.strMemo = "验证码不正确";
					}
					else
					{
						rgPack.strMemo = "未知错误";
