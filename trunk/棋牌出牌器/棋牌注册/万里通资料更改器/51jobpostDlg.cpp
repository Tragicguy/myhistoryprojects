...
typedef int (FAR __stdcall *FnMatchCheckCode)(char *szPath, char *szCode,int iThreadShod);
...
BOOL CjobpostDlg::OnInitDialog()
{
	...
	strAppPath += "CodeCheck.dll";
	m_handle = LoadLibrary(strAppPath);//
	if(m_handle == 0x0)
	{
		MessageBox("加载CodeCheck.dll失败","错误");
		exit(0);
	}
...
LRESULT CjobpostDlg::AddToList(WPARAM wParam,LPARAM lParam)
{
	...
	else if (msgBuf == "获取验证码成功")
	{
		...
		if (g_iAotuCheckCode)
		{
			...
			MatchCheckCode(szPath,szCheckCode,240);
