...
CString g_strWindowName = "贴贴狂帖-百度贴吧顶贴机 V1.5 试用版";
CString g_strWindowClass = "ThunderRT6FormDC";
CString g_strAddContentName  = "发贴内容";
...
void CLoserWindowDlg::RunFunc()
{
	...
	CWindowFind find;
	CString strTemp = "";	
	ShowMessage("初始化成功,开始添加数据到目标程序中");
	while (!g_ExitFlag)
	{	
		if (g_iWinOrLose = 1)
		{
		...
		strTemp.Format("完成  %s  的添加",rgPack.strAcc);
		ShowMessage(strTemp);
		strTemp.Format("等待%d秒",g_iCheckGmWaitInterval);
		ShowMessage(strTemp);
...
int CLoserWindowDlg::TryIntoRoom()
{
	...
	if (hParant)
	{
		...
		if (hwnd && find.GetContrlClassName(hwnd) =="SysTreeView32")	//登录框的子窗口
		{
			...
			HTREEITEM hChild = FindItem(hwnd,hItem,"扎金花");
			if (!hChild)
			{
				ShowMessage("没找到扎金花房间");
				return -1;
			}
			hChild = FindItem(hwnd,hChild,"普通房间1");
			if (!hChild)
			{
...
int CLoserWindowDlg::TryIntoDesk(int iIndex,int iSetIndex)
{
	...
	{//找到房间
		...
		{			
			strTemp.Format("扎金花游戏客户端 ［ 普通房间1 - 第 %d 桌 ］",iIndex);
...
int CLoserWindowDlg::PlayGame()
{
	...
	if (hwnd)
	{
		...
		while(true)
		{
			...
			strTemp.Format("扎金花游戏客户端 ［ 普通房间1 - 第 %d 桌 ］",g_iRoomNum);
...
void CLoserWindowDlg::SetTableStyle()
{
	...
	conList->InsertColumn(1,"文件名",LVCFMT_LEFT,150);
	conList->InsertColumn(2,"图片地址",LVCFMT_LEFT,150);
	conList->InsertColumn(3,"状态",LVCFMT_LEFT,80);
}
/************************************************************************/
/*                                                                      */
/************************************************************************/
...
// 			break;
	...
	finder.Close();
	CString strShow = "";
	strShow.Format("共导入了%d个文件",iCount);
	ShowMessage(strShow);
}
/************************************************************************/
/*                                                                      */
...
int CLoserWindowDlg::SaveParams()
{
	...
	{
		MessageBox("窗口相关信息或数据文件信息不能为空","警告");
		return -1;
	}
...
void CLoserWindowDlg::AddFreshResult(RegisterPack rgPack,int iFlag)
{
	...
	for (int i=0;i<nCount;i++)
	{
		...
		{
			pList->SetItemText(i,3,iFlag==0?"成功":"失败");
			break;
		}
	}
}
...
int CLoserWindowDlg::GetWindowCtrl()
{
	...
	if (!g_hwndWindow)
	{
		ShowMessage("未找到目标窗口,请确定目标程序启动并且您的配置正确");
		return -1;
	}
	CWindowFind find;
