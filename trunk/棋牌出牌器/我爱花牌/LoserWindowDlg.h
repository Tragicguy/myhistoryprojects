...
#include <map>
using namespace std;
#import "HookCall.ocx" //rename_namespace("OCRXX")
using namespace HookCall;
...
public:
	...
	bool InitHuapaiPlay();
	bool IsWaitting(HWND hwnd);
	void QuitPlayWindow();
	HWND GetPlayDesktopWindow();
	void CloseHuaPaiWarnWidow();
	void SaveMoneyToBank(int iMoney,CString strPwd);
	bool GetMoneyFromBank(long iMoney,CString strPwd);
	bool IsBoss();
	afx_msg void OnTimer(UINT_PTR nIDEvent);
	void Blockbet(int iFlag,int iMoney);
	void ReadPlayData(CString &strName,int &iScore,int &iWinMoney,int &iPlayNum, HANDLE &hProcess,long lDataAddr);
	_UserControl1 *m_AddrCall;
	afx_msg void OnEnChangeEditPlaynum();
	afx_msg void OnEnChangeEditSetnum2();
	afx_msg void OnEnChangeEditScore();
	afx_msg void OnEnChangeEditMybalance();
	afx_msg void OnEnChangeEditGetkeep();
	int GetPalyType();
	void SetPalyType(int iFlag);
	afx_msg void OnBnClickedRadioPease();
	afx_msg void OnBnClickedRadioPeasesame();
	afx_msg void OnBnClickedRadioWinleft();
	afx_msg void OnBnClickedRadioWinright();
};