#include "stdafx.h"
//#pragma comment(lib,"downloadmgr.lib")
#if _MFC_VER >= 0x0700
#include <afxocc.h>
#else
#ifdef _AFXDLL
...
CDocHostSite::CDocHostSite(COleControlContainer * pOcc, CWeb1800HtmlView* pView)
...
HRESULT FAR EXPORT CDocHostSite::XDownloadManager::QueryInterface(REFIID riid, void** ppvObj)
{
	METHOD_PROLOGUE(CDocHostSite, DownloadManager)
	HRESULT hr = (HRESULT)pThis->ExternalQueryInterface(&riid, ppvObj);
	return hr;
}

ULONG FAR EXPORT CDocHostSite::XDownloadManager::AddRef()
{
	METHOD_PROLOGUE(CDocHostSite, DownloadManager)
		return pThis->ExternalAddRef();
}

ULONG FAR EXPORT CDocHostSite::XDownloadManager::Release()
{                            
...
HRESULT CDocHostSite::XDownloadManager::Download( 
								   /* [in] */ IMoniker *pmk,
								   /* [in] */ IBindCtx *pbc,
								   /* [in] */ DWORD dwBindVerb,
								   /* [in] */ LONG grfBINDF,
...
// 	if (guidService == SID_SNewWindowManager &&
// 		riid == IID_INewWindowManager)
// 	{
// 		METHOD_PROLOGUE(CDocHostSite, ServiceProvider);
// 		HRESULT hr = (HRESULT)pThis->ExternalQueryInterface(&riid, ppvObject);
// 		return hr;
// 	}
	else if (guidService == SID_SDownloadManager && riid == IID_IDownloadManager)
	{
		/*CComObject<XDownloadManager>* pDownloadMgr;
		HRESULT hr = CComObject<XDownloadManager>::CreateInstance(&pDownloadMgr);
		if( (SUCCEEDED(hr)) || (pDownloadMgr) )
		{
			return pDownloadMgr->QueryInterface(IID_IDownloadManager, ppvObject);
		}*/
		//METHOD_PROLOGUE(CDocHostSite, DownloadManager)
		//HRESULT hr = (HRESULT)pThis->ExternalQueryInterface(&riid, ppvObject);
// 		int iii = _WIN32_IE;
// 		 //Create new CDownloadMgr object using ATL.
 		//XDownloadManager pDownloadMgr;// = new XDownloadManager;
		//pDownloadMgr.QueryInterface(riid,ppvObject);
// 		HRESULT hr = CComObject<DownloadManager>::CreateInstance(&pDownloadMgr);
// 
