namespace Manzana
{
    ...
    {
        private static DirectoryInfo ApplicationSupportDirectory = new DirectoryInfo(Registry.GetValue(@"HKEY_LOCAL_MACHINE\SOFTWARE\Apple Inc.\Apple Application Support", "InstallDir", Environment.CurrentDirectory).ToString());
        private const string DLLName = "iTunesMobileDevice.dll";
        private static FileInfo iTunesMobileDeviceFile = new FileInfo(Registry.GetValue(@"HKEY_LOCAL_MACHINE\SOFTWARE\Apple Inc.\Apple Mobile Device Support\Shared", "iTunesMobileDeviceDLL", "iTunesMobileDevice.dll").ToString());
        public static void SetInfo(string strApplicationSupportDirectory, string striTunesMobileDeviceFile)
        {
            ApplicationSupportDirectory = new DirectoryInfo(strApplicationSupportDirectory);
            iTunesMobileDeviceFile = new FileInfo(striTunesMobileDeviceFile);
        ...
        static MobileDevice()
        {
            ...
            if (!iTunesMobileDeviceFile.Exists)
            {
                ...
                {
                    directoryName = @"C:\Program Files (x86)\Apple\Mobile Device Support\bin";
                }
            }
