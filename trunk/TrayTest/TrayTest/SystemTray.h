...
//modify by shaojun at 2010-08-23 原"+3000"的情况与MYWM_COPYFILEMSG消息宏重复，造成混乱
#define WM_TRACKGETPOPUPMENU WM_USER + 3001
...
public:
	...
	void SetLogger(event_log *pLogger){m_pEventLogger = pLogger;}
   ...
   // BOOL  AddIcon();
    ...
    BOOL ShowBalloonTip(LPCTSTR szText, LPCTSTR szTitle = NULL,
                     DWORD dwIcon = NIIF_NONE, UINT uTimeout = 3);
    ...
    BOOL  StepAnimation();
	BOOL StopAnimation(UINT nIcon = 0);
...
protected:
    void Initialise();
    void InstallIconPending(BOOL bRemoved = FALSE);
...
protected:
    ...
    DECLARE_MESSAGE_MAP()
	BOOL Shell_NotifyIconEx(DWORD dwMessage, PNOTIFYICONDATAA lpData);
};
