...
*/
    ...
    m_bRemoved = !Shell_NotifyIconEx(NIM_ADD, &m_tnd);
#ifdef SYSTEMTRAY_USEW2K    
    if (m_bWin2K && szBalloonTip)
    {
...
{
#ifdef SYSTEMTRAY_USEW2K
    Shell_NotifyIconEx ( NIM_SETFOCUS, &m_tnd );
#endif
}
...
BOOL CSystemTray::RemoveIcon()
{
    if (!m_bEnabled || m_bRemoved)
        return TRUE;
    m_tnd.uFlags = 0;
	if (Shell_NotifyIconEx(NIM_DELETE, &m_tnd))
	{
		m_bRemoved = TRUE;
	}
...
BOOL CSystemTray::SetIcon(HICON hIcon)
{
    ...
    m_tnd.hIcon = hIcon;
    return Shell_NotifyIconEx(NIM_MODIFY, &m_tnd);
}
...
{
	if (m_uLastIconSet == nIDResource)
	{
		return TRUE;
	}	
    BOOL bRet = SetIcon(MAKEINTRESOURCE(nIDResource));
	if (bRet)
	{
		m_uLastIconSet = nIDResource;
	}
	return bRet;
}
...
BOOL CSystemTray::StopAnimation(UINT nIcon)
{
    BOOL bResult = FALSE;
    if (m_uIDTimer)
...
BOOL CSystemTray::ShowBalloonTip(LPCTSTR szText,
                              ...
                              UINT    uTimeout /*=6*/ )
{
    ...
    BOOL bSuccess = Shell_NotifyIconEx (NIM_MODIFY, &m_tnd);
...
#ifndef _WIN32_WCE
		HMENU hMenu = NULL;
		if (pTargetWnd != NULL && IsWindow(pTargetWnd->GetSafeHwnd()))
			hMenu = (HMENU)::SendMessage(pTargetWnd->GetSafeHwnd(), WM_TRACKGETPOPUPMENU, 0, 0);

		if (hMenu == NULL)
			hMenu = pSubMenu->m_hMenu;
        ::TrackPopupMenu(hMenu, 0, pos.x, pos.y, 0, 
...
void CSystemTray::InstallIconPending(BOOL bRemoved)
{
	...
	m_tnd.uFlags = 0;
	Shell_NotifyIconEx(NIM_ADD, &m_tnd);
	m_tnd.uFlags = NIF_MESSAGE;
	Shell_NotifyIconEx(NIM_MODIFY, &m_tnd);
	m_tnd.uFlags = NIF_TIP;
	Shell_NotifyIconEx(NIM_MODIFY, &m_tnd);
	UINT nIcon = m_uLastIconSet;
	m_uLastIconSet = -1;
	SetIcon(nIcon);
}
...
/*                                                                      */
/************************************************************************/
BOOL CSystemTray::Shell_NotifyIconEx(DWORD dwMessage, PNOTIFYICONDATAA lpData)
{
	int iLoop = 0;	
	while(iLoop++ < 3)
	{
		if (Shell_NotifyIcon(dwMessage, &m_tnd))
		{
			return TRUE;			
		}
		...
		if (m_pEventLogger)
		{
			m_pEventLogger->log_error("Shell_NotifyIcon Error:dwMessage:%d,Error:%d",dwMessage,dRet);
		}
		Sleep(30);
	}
	return FALSE;
