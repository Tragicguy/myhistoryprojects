...
//             }
        ...
        //需要时候显示滚动条
            ...
            listView.Columns.Add("接受者", 120);
            listView.Columns.Add("卡号", 160);
            listView.Columns.Add("密码", 100);
        ...
        private void AddViewItem(string strAcc,string strPwd,string strRecver)
        {
            ListViewItem li = new ListViewItem(); 
            li.SubItems.Clear();
            li.SubItems[0].Text = listView.Items.Count.ToString();
            li.SubItems.Add(strRecver);
        ...
        private void ModifyViewItem(_GoodsFreshStr goodInfo)
        {
            ...
            {
                if (listView.Items[i].SubItems[2].Text.ToString() == goodInfo.strAcc)
                {
                    listView.Items[i].SubItems[4].Text = goodInfo.strFreshStatus;                   
                    break;
                }                
            }
        }
...
//             m_Timer.Start();
        ...
        void ImportAcc(string strPath,string strRecver)
        {
            ...
            foreach (string strLine in strArray)
            {
                ...
                GoodInfo.strModifyStatus = strRecver;
              //  ShowLogString(string.Format("正在加载 {0} 的商品同步信息", GoodInfo.strAcc));
                ...
                AddViewItem(GoodInfo.strAcc, GoodInfo.strPwd,GoodInfo.strModifyStatus);
        ...
        {
            string strRecver = textFreshStart.Text;
            if (strRecver == "")
            {
                MessageBox.Show("请先填写接收者");
                return;
            }
            string strPath = "";
        ...
        private string GetCheckCode(string strPath)
        {
            string strShow = "";
            lock(m_hLockTable)
            {
                ...
                string strDirect = System.AppDomain.CurrentDomain.BaseDirectory;
                strDirect += "tessdata";
                ocr.Init(strDirect, "eng", false); //应用当前语言包。注，Tessnet2是支持多国语的。语言包下载链接：http://code.google.com/p/tesseract-ocr/downloads/list
