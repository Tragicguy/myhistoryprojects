...
BOOL CjobpostDlg::OnInitDialog()
{
	...
	m_handle = 0x0;
	SetWindowText("Trion扫号器");
	gl_hwnd = m_hWnd;
	SetDlgItemInt(IDC_EDIT_SUCNUM,0);
	SetDlgItemInt(IDC_EDIT_TOTAL,0);
	HWND hwnd = ::FindWindow("#32770","AmazonRobot");
	if (hwnd)
	{
...
// 	hInstDLL= LoadLibrary(_T("Vcode.dll"));	//载入同目录下的Vcode.dll
// 	if(hInstDLL)
// 	{
...
{
	//SetDlgItemInt(IDC_EDIT_TOTAL,g_RegisterList.GetCount());
	g_ExitFlag = false;
...
void CjobpostDlg::SetTableStyle()
{
	...
	conList->InsertColumn(1,"账名",LVCFMT_LEFT,150);
	conList->InsertColumn(2,"密码",LVCFMT_LEFT,150);
	conList->InsertColumn(3,"邮箱",LVCFMT_LEFT,0);
	conList->InsertColumn(4,"身份证号",LVCFMT_LEFT,0);
	conList->InsertColumn(5,"状态",LVCFMT_LEFT,100);
	conList->InsertColumn(6,"备注",LVCFMT_LEFT,100);
	conList->InsertColumn(7,"推荐人",LVCFMT_LEFT,0);
...
// 				g_strCheckCode = LPCTSTR(strRet);
				...
				CString str;//识别结果
				GetFileVcode((LPTSTR)(LPCSTR)(g_strDirect + BMPFILE_PATH),1,str.GetBuffer(100));//划分缓冲区
				str.ReleaseBuffer ();//释放缓冲区
				g_strCheckCode = str;//GetVcode((LPTSTR)(LPCSTR)(g_strDirect + BMPFILE_PATH),1);//识别图片码
			}
			catch (...)
			{
				g_strCheckCode = "";
...
void CjobpostDlg::OnBnClickedButton2()
{
	if (g_ExitFlag == false)
	{
		AfxMessageBox("请先停止线程再执行清空");
		return;
	}
	g_AccList.DeleteAllItems();
	gl_mapListAcc.clear();
	m_message.DeleteAllItems();
	SetDlgItemInt(IDC_EDIT_SUCNUM,0);
	SetDlgItemInt(IDC_EDIT_TOTAL,0);
	SetDlgItemInt(IDC_EDIT_FAILCOUNT,0);
	return;
	CRasDialMgr mgr;
	mgr.SetVpnInfo("98.126.33.121","d","rfth","rfth");
	mgr.ConnectVPN();
return;
...
/*                                                                      */
/************************************************************************/
LRESULT CjobpostDlg::DefWindowProc(UINT message, WPARAM wParam, LPARAM lParam)
{
	try
	{
		switch (message)
		...
		case WM_USER + 111:
			{
				int iCount = GetDlgItemInt(IDC_EDIT_SUCNUM);
				SetDlgItemInt(IDC_EDIT_SUCNUM,iCount+(int)lParam);				
				SetDlgItemInt(IDC_EDIT_FAILCOUNT,GetDlgItemInt(IDC_EDIT_FAILCOUNT)+(int)wParam);
...
int CjobpostDlg::GetAppPath(CString &strAppPath)
{
	int iRet = -3;
	try
	{
...
int CjobpostDlg::GetPower()
{
	...
	try
	{
		CStdioFile *pFile = session.OpenURL("http://blog.163.com/shaojun_hnist/blog/static/1387171242011115387766/?latestBlog");
		if (pFile != NULL)
		{
			while(pFile->ReadString(strTemp))
			{
				strTemp.MakeLower();
				if (strTemp.Find("myownpowetrion") !=-1)
				{
					pFile->Close();
					delete pFile;
					pFile = NULL;
