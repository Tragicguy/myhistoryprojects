...
bool C51JobWebPost::UserRegister()
{
	try
	{
		...
		//检查是否打开简历管理页
		int iPos = m_strWorkBuf.Find("请输入右图显示的四个数字");
		if (iPos >= 0)
		{
			ShowMessage("成功获取基金页,正在获取验证码...");
		}
		else
		{
			return false;
...
int C51JobWebPost::UserRegisterEx(RegisterPack &rgPack)
{
	try
	{
		...
		g_strGotNum = "";
		ShowMessage("获取基金页面完成,开始解析...");
		//此处获取已经获取数、是否还能继续获取等信息
		if (m_strWorkBuf.Find("获取用户信息失败")>=0)
		{
			strFailReason = "游戏ID或者密码错误";
			return ErrorInfoShow(rgPack,strFailReason);
		}
		//获取参数失败
		if (m_strWorkBuf.Find("获取参数失败")>=0)
		{
			strFailReason = "游戏ID或者密码非法";
			return ErrorInfoShow(rgPack,strFailReason);
		}
		//此处继续处理账号非法信息
		...
		if (g_strGotNum == "3")	//表示已经获取完成
		{	
			ShowMessage("此账号已经获取过三次基金");
			return ErrorInfoShow(rgPack,"此账号已经获取过三次基金");
		}
// 		if (m_strWorkBuf.Find("您现在的在线游戏时间没达到领取标准") >= 0)
// 		{
// 			ShowMessage("您现在的在线游戏时间没达到领取标准");
// 			return ErrorInfoShow(rgPack,"您现在的在线游戏时间没达到领取标准");
// 		}
		if (m_strWorkBuf.Find("请输入右图显示的四个数字") >= 0)
		{
			ShowMessage("成功获取基金页,正在获取验证码...");
		}
		else
		{
			ShowMessage("解析基金页面出错[未找到标识]");
			return -1;
		}
		//此处处理不同的显示值来判断是否可以进行获取基金
		...
		if (m_iCheckFlag == 0)
		{
			...
			{
				ShowMessage("获取验证码成功");	
				strShow.Format("正在获取基金:%s %s",rgPack.strAcc,rgPack.strPwd);
				ShowMessage(strShow);
			}
		}		
		while(!g_ExitFlag)
...
{
	ShowMessage("获取失败:	"+strFailReason);
	rgPack.iStatus = 2;
	rgPack.strMemo = strFailReason;
	rgPack.strIp = g_strGotNum;		//表示已经获取的次数 
	if (rgPack.strIp == "")
	{
		rgPack.strIp = "-";
	}	
	//游戏ID或者密码非法
	if (strFailReason.Find("游戏ID或者密码非法")>=0)
	{
		return -2;
	}
	//游戏ID或者密码错误
	if (strFailReason.Find("游戏ID或者密码错误")>=0)
	{
		return -2;
	}
	if (strFailReason.Find("此账号已经获取过三次基金")>=0)
	{
		return -2;
	}
	if (strFailReason.Find("您现在的在线游戏时间没达到领取标准")>=0)
	{
		return -2;
	}
	//您还未达到领取亿酷基金的条件
	if (strFailReason.Find("您还未达到领取亿酷基金的条件")>=0)
	{
		return -2;
	}
	//验证码输入不正确
	if (strFailReason.Find("验证码输入不正确")>=0)
	{
		return -1;
	}
	//请输入四个数字验证码
	if (strFailReason.Find("请输入四个数字验证码")>=0)
	{
		return -1;
	}
