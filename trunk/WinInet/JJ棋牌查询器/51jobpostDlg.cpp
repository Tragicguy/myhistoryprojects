...
void CjobpostDlg::SetTableStyle()
{
	...
	conList->InsertColumn(1,"游戏账名",LVCFMT_LEFT,100);
	conList->InsertColumn(2,"密码",LVCFMT_LEFT,100);
	conList->InsertColumn(3,"账户金币",LVCFMT_LEFT,150);
	conList->InsertColumn(4,"保险箱金币",LVCFMT_LEFT,150);	
	conList->InsertColumn(5,"密码保护",LVCFMT_LEFT,100);
	conList->InsertColumn(6,"状态",LVCFMT_LEFT,150);
}
/************************************************************************/
/*                                                                      */
/************************************************************************/
...
int CjobpostDlg::GetPower()
{
	...
	try
	{
		...
		if (pFile != NULL)
		{
			...
			{
				strTemp.MakeLower();
				if (strTemp.Find("myownpowerjjquery") !=-1)
				{
					pFile->Close();
					delete pFile;
					pFile = NULL;
