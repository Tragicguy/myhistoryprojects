...
void AddToShowList(RegisterPack rgPack)
{
	...
	{
		if (g_AccList.GetItemText(i,1) == rgPack.strAcc&&g_AccList.GetItemText(i,2) == rgPack.strPwd)
		{
			isExist = true;
			g_AccList.SetItemText(i,5,rgPack.strIp);
			g_AccList.SetItemText(i,6,rgPack.strMemo);
			if (rgPack.iStatus == 1)
			{
				g_AccList.SetItemText(i,3,rgPack.strMoney);
				g_AccList.SetItemText(i,4,rgPack.strBank);
			}
			break;
		}		
	}
...
int C51JobWebPost::UserRegisterEx(RegisterPack &rgPack)
{
	try
	{
		m_strServerURL = "jj.cn";
		CString strError = "";
		CString sendStr = "";
		ShowMessage("正在获取登录页面...");
		CString objectlink = "/user/login.php?comeurl=http%3A%2F%2Fmy.jj.cn%2Fcoffer.php";
		CString strHeaders = m_strHeader;
		int iRet = RequestGetPageExExEx(m_strServerURL,objectlink,strHeaders,strError);
		if ( iRet != 0 )
		{
		...
		int iPos = m_strWorkBuf.Find("<title>用户登录</title>");
		if (iPos >= 0)
		{
			ShowMessage("成功获取登录页,登录...");
		}
		else
		{
			return -1;
		...
		if ( iPos >= 0)
		{
			...
			if (m_strWorkBuf.Find("您还没有设置密码保护") >0)
			{
				rgPack.strIp = "无";
			}
		}
		...
		/*
		strHeaders = m_strHeader + "\r\nReferer:http://jj.cn/user/login.php";
		strBefore = "src=\"";
		strAfter = "\"";
		CString strUrl = GetKeyString(m_strWorkBuf,strBefore,strAfter);

		strBefore = "http://";
		strAfter = "/";
		CString strHost = GetKeyString(strUrl,strBefore,strAfter); 
		m_strServerURL = strHost;
		strBefore = ".cn/";
		iPos = strUrl.Find(strBefore)+strBefore.GetLength();
		objectlink = strUrl.Mid(iPos);
		//DeleteObjectExEx();
...
void C51JobWebPost::RegisterFunc()
{
	...
	while (!g_ExitFlag)
	{
		...
		if (iRunCount >= g_iFreshCount)
		{
			ShowMessage("单个IP登录数已经达到最大，请更换IP...");
			ShowMessage("stop");
			break;
		}
	}
