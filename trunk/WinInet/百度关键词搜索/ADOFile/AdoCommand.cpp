...
_RecordsetPtr CAdoCommand::Execute(int &iRet,long Options)
{
	ASSERT(m_pCommand != NULL);
	try
	{
	...
	catch (_com_error e)
	{
		...
		strException.Format("Execute 发生异常. 错误信息: %s; 文件: %s; 行: %d---%s",e.ErrorMessage(), __FILE__, __LINE__, strErrorState);
#ifdef _DEBUG
...
_variant_t CAdoCommand::GetValue(LPCTSTR lpstrName)
{
	...
	try
	{
		 _variant_t aa = m_pCommand->GetParameters()->GetItem(_variant_t(lpstrName))->Value;
		 return aa;
	}
	catch (_com_error e)
	{
