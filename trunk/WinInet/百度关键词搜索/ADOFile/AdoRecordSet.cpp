...
BOOL CAdoRecordSet::Open(LPCTSTR strSQL, long lOption, CursorTypeEnum CursorType, LockTypeEnum LockType)
{
	...
	catch (_com_error e)
	{
		...
		//CString strErrorState = GetLastError();
		//strException.Format("打开记录集发生异常. 错误信息: %s; 文件: %s; 行: %d---%s",e.ErrorMessage(), __FILE__, __LINE__, strErrorState);
		
// #ifdef _DEBUG
// 		if (strErrorState == "08S0 " || strErrorState == "08001" || strErrorState == "获取错误信息时异常")	//关闭连接
// 		{
...
// #endif		
 	...
 	{
// 		CString strException = "";
// 		strException.Format("打开记录集发生异常. 错误信息: %s; 文件: %s; 行: %d---%s","未知错误", __FILE__, __LINE__,"未知错误;");
// 		m_pConnection->Close();		//关闭数据库连接
		return FALSE;
	}
}
...
/************************************************************************/
//shaojun add
void CAdoRecordSet::SetRecordSetPtr(_RecordsetPtr pRecordset)
{
	m_pRecordset = pRecordset;
}