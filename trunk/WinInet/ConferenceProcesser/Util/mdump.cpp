...
long CMiniDumper::MyUnhandledExceptionFilter(struct _EXCEPTION_POINTERS* pExceptionInfo)
{
	long ret = EXCEPTION_CONTINUE_SEARCH;
	...
	char szTbuffer[_MAX_PATH] = {0};
	//sprintf(szTbuffer, "%d%d%d%d", t.GetDay(), t.GetHour(), t.GetMinute(), t.GetSecond());
	lstrcpy(szTbuffer, "MyHandled");
	...
	if (GetModuleFileName(NULL, szPath, _MAX_PATH))
	{
		...
		_tcsncat(szDumpPath, szDir, _MAX_PATH - _tcslen(szDumpPath) - 1);
		_tcsncat(szDumpPath, szFileName, _MAX_PATH - _tcslen(szDumpPath) - 1);
		_tcsncat(szDumpPath, szTbuffer, _MAX_PATH - _tcslen(szDumpPath) - 1);
		_tcsncat(szDumpPath, _T(".dmp"), _MAX_PATH - _tcslen(szDumpPath) - 1);
	}
