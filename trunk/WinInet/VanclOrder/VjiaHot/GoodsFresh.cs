...
#region PAOPAO
        ...
        public bool HotOrder(_GoodsFreshStr goodInfo, string strOrderUrl)
        {
            string strReffer = "http://mkt.vjia.com/html/2012/11/xiaomi1s2p/index.html";
            string strOrderPage = "http://mkt.vjia.com/html/2012/11/xiaomi1s2p/default.aspx";
           
            string strRet = m_webPost.GetHttpPage(strOrderPage,
                    "utf-8", "get", "", "",
                    strReffer,
                    "*/*", "");

            string strCheckCode = "";
            string strCheckCodeUrl = string.Format("http://www.vjia.com/login/process/VerifyCode.ashx?t={0}", DateTime.Now.ToFileTime());
            m_objForm.GetCheckCode(m_webPost, "", out goodInfo.strFreshStatus, out strCheckCode, strCheckCodeUrl, strOrderPage);

            string strRealOrderUrl = string.Format(strOrderUrl, strCheckCode);
            strRet = m_webPost.GetHttpPage(strRealOrderUrl,
                    "utf-8", "get", "", "",
                    strOrderPage,
                    "*/*", "");
            strRet = strRet.ToLower();
            if (strRet.IndexOf("\"isok\":true") > 0)
            {
                return true;
            }
            goodInfo.strFreshStatus = strRet.Length > 100 ? strRet.Substring(0, 100) : strRet;
            return false;
        }
