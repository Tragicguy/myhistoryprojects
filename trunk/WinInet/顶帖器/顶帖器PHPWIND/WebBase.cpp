...
CWebBase::CWebBase( )
{
	...
	m_nElseWhereIP = 0;
	m_iHttpsFlag = 0;
	memset(m_szWorkBuf,0,sizeof(m_szWorkBuf));
}
/************************************************************************/
/*                                                                      */
/************************************************************************/
...
/*                                                                      */
/************************************************************************/
void CWebBase::ConvertUtf8ToGBKEx() 
{
	int len = 0;
	unsigned short * wszGBK = NULL;
	char * szGBKt = NULL;
	...
	len = MultiByteToWideChar(CP_UTF8, 0, (LPCTSTR)m_szWorkBuf, -1, NULL,0);
	wszGBK = new unsigned short[len+1];
	memset(wszGBK, 0, len * 2 + 2);
	MultiByteToWideChar(CP_UTF8, 0, (LPCTSTR)m_szWorkBuf, -1, (LPWSTR)wszGBK, len);
	len = WideCharToMultiByte(CP_ACP, 0, (LPWSTR)wszGBK, -1, NULL, 0, NULL, NULL); 
	szGBKt  =new char[len + 1];
	memset(szGBKt, 0, len + 1);
	WideCharToMultiByte (CP_ACP, 0, (LPWSTR)wszGBK, -1, szGBKt, len, NULL,NULL);
			...
			StatusCode == HTTP_STATUS_REDIRECT_METHOD)
		{
			...
			if (iGetType == 0)			//读取文本信息
			{
				while( iReadLen >= 0 )
				{
					...
					iReadLen = pHttpFile->Read( cBuffer ,500 );
					if ( iReadLen <= 0)
					{
						break;
					}
			...
			StatusCode == HTTP_STATUS_REDIRECT_METHOD)
		{
			...
			if (iGetType == 0)			//读取文本信息
			{
				while(iReadLen >= 0)
				{
					...
					//memcpy(m_szWorkBuf,cBuffer,m_iszWorkBufLen);
					//m_iszWorkBufLen+=iReadLen;
					m_strWorkBuf += CString( cBuffer ,iReadLen);
				}
			}
			else		//读取验证码
