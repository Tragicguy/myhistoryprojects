...
// 		}
		...
		if (g_iIsNeedLogin == 1)
		{
			...
			file22.Close();
			int iPos = m_strWorkBuf.Find("如果您设置了安全提问，请回答正确的答案");			
			if (iPos >= 0)
			{
				ShowMessage("成功获取登录页...");
			}
			else if (m_strWorkBuf.Find("欢迎您回来") > 0)	//需要退出登录
			{
				objectlink = "/index.php";
				Sleep(iDelay);
				iRet = RequestGetPageExExEx(m_strServerURL,objectlink,strHeaders,strError);
				//<a href="logging.php?action=logout&amp;formhash=ac10e1b2" class="notabs">退出</a>
				CString strBefore = "";
				CString strAfter = "\" class=\"notabs\">退出";
			...
			file.Close();
			CString strFind = "";
			strFind.Format( "欢迎您回来，%s。",strUserName);
