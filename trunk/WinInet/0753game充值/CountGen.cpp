...
BOOL CCountGen::OnInitDialog()
{
	...
	m_AutoCheck.ShowWindow(SW_SHOW);
...
void CCountGen::OnBnClickedButton1()
{
	...
	if (strPhone == "")
	{
		MessageBox("待充值账号不能为空,请一定填写您要充值的账号,切勿充错!");
		return;
	}
	if (SaveParams() < 0)
	{
...
// 			}
			...
			if (strAcc !="" && strPwd != "" && strPhone != "")
			{
				...
				{
					if (strAcc == m_pList->GetItemText(i,2) && strPwd == m_pList->GetItemText(i,3))
					{
						CString strShow = "";
				...
				m_pList->InsertItem(nCount,"",0);
				m_pList->SetItemText(nCount,0,listIndex);
				m_pList->SetItemText(nCount,1,strPhone);
				m_pList->SetItemText(nCount,2,strAcc);
				m_pList->SetItemText(nCount,3,strPwd);				
			}
		}
		file.Close();
	}
