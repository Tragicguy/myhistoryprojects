...
BOOL CCountGen::OnInitDialog()
{
	...
	::SetWindowPos(m_hWnd,HWND_TOP,0,0,405,250,~SWP_NOSIZE & SWP_SHOWWINDOW|SWP_NOMOVE);
	//读取配置
	CString strAppPath = "";
	CString strTemp = "";
...
// 	}
	...
	g_iFreshCount = atoi(strFileName);
	strDirect.Trim();
	strFileName.Trim();
...
void CCountGen::OnBnClickedButton1()
{
	...
	if (strPhone == "")
	{
		MessageBox("查询时间间隔不能为空,请一定填写!");
		return;
	}
	g_iFreshCount = atoi(strPhone);	//时间间隔
	if (SaveParams() < 0)
	{
		return;
	}
...
// 			strPwd = cstr.Left(idx2);
			...
			map<CString,int>::iterator it = m_mapOldAcc.find(strAcc);
			if (it != m_mapOldAcc.end())
			{
				CString strShow = "";
				strShow.Format("%s,%s 已经对此域名做过处理",strAcc,strPwd);
				CUserMessagePump *pMsgPump = NULL;
				pMsgPump = CUserMessagePump::GetInstance();
				if (!pMsgPump)
				{
