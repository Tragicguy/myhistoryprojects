...
#region PAOPAO
        ...
        public bool PaoPaoLogin(string strAcc, string strPwd, string strCode, out string strError, bool bQuickFresh)
        {
            ...
            try
            {
                string strData = string.Format("backUrl=&userid={0}&pwd={1}&check_key={2}",
                strAcc,
                strPwd,
                strCode);
                string strRet = m_webPost.GetHttpPage("http://mydealer.zol.com.cn/admin/index.php?action=login",
                    "gb2312", "post", "", strData,
                    "http://dealer.zol.com.cn/merchant_login.php",
                    "*/*", "");
                if (strRet == "")
                {
                    strError = "登录失败,网络错误";
                ...
                if (strRet.IndexOf("验证码不正确") > 0)
                {
                    strError = "验证码错误";
                    return false;
                }
                if (strRet.IndexOf("密码错误")> 0)
                {
                    strError = "用户名或密码错误";
                    return false;
                }
                if (strRet.IndexOf("用户名错误")>0)
                {
                    strError = "用户名错误";
                    return false;
                }
                if (strRet.IndexOf("document.location.href='index.php';") < 0)
                {
                    strError = "登录失败";
                    return false;
                }
                ...
                if (bQuickFresh)
                {
                    strUrl = "http://mydealer.zol.com.cn/admin/index.php";
                }
                else{

                    strUrl = "http://mydealer.zol.com.cn/admin/index.php?action=price&pro_type=price&ol_type=ol_price&subcatid=0&manuid=0&search_type=&keyword=&page=1";
                }
                //登录成功
                strRet = m_webPost.GetHttpPage(strUrl,
                    ...
                    "",
                    "*/*", "");
                if (!(strRet.IndexOf("你好") > 0 && strRet.IndexOf(strAcc) > 0))
                {
                    strError = "登录失败";
                    return false;
                }
        ...
        public int FreshQuick(out string strError,out string strLeft)
        {
            ...
            try
            {
                //string strTemp = m_strGoodContent.Substring(m_strGoodContent.IndexOf("function updateData(){"));
...
//                 }
                //>一键刷新(9次)</span>
                string strBefore = ">一键刷新(";
                string strAfter = "次)</span>";
                ...
                strUrl = string.Format("http://mydealer.zol.com.cn/admin/include/update_price_time.php?updatetime={0}", dt2JsTicks);//
                //1299041534823
                strReffer = "http://mydealer.zol.com.cn/admin/index.php";
#else
#endif
                string strRet = m_webPost.GetHttpPage(strUrl,
                    "utf-8", strType, "", strData,
                    strReffer,
                    "*/*", "");


#if __PAOPAO
               if (strRet.IndexOf("刷新成功") >= 0)
               {
                   return 0;
               }
               else if (strRet.IndexOf("一个经销商帐号只能在一台机器上登录")>=0)
               {
                   strError = "一个经销商帐号只能在一台机器上登录";
               }
               else if (strRet.IndexOf("距上次刷新时间间隔过短，本次刷新无效，请稍后再试")>=0)
               {
                   strError = "距上次刷新时间间隔过短，本次刷新无效，请稍后再试";
               }
               else{
                   strError = "刷新失败";
               }
...
#if __PAOPAO
                strUrl = string.Format("http://mydealer.zol.com.cn/admin/index.php?action=price&pro_type=price&ol_type=ol_price&subcatid=0&manuid=0&search_type=&keyword=&page={0}", iCurPage);
                strCompleteFlag = "";
                AnalyzeGoodsPaoPao(m_strGoodContent, ref hSynTable, ref hSynDuplicateTable, iCurPage - 1, strMid);
                //页次<font color='red'>1</font>/69 共
                strTemp = strTemp.Substring(strTemp.IndexOf("页次"));
                string strNumLeft = GetKeyString(strTemp, "<font color='red'>", "<").Trim();
                string strNumRight = GetKeyString(strTemp, "</font>/", " ").Trim();
