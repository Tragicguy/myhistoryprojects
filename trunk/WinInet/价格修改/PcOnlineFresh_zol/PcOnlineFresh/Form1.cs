...
namespace PcOnlineFresh
{
    ...
    public partial class Form1 : Form
    {
        ...
        private void TimerFunction()
        {
            ...
            if (i < m_iRefreshInterval)   //未到刷新间隔
            {
                return;
            }            
            ...
            foreach (string strLine in strArray)
            {
                ...
                else
                {
                    ...
                    foreach (string strGoodInfo in strArrayEx)
                    {
                        ...
                        goodModify.strMid = strSynInfo[5];
                        goodModify.strPay = strSynInfo[6];
                        goodModify.strSequence = strSynInfo[7];
                        goodModify.strTicket = strSynInfo[8];
                        goodModify.strTrueprice = strSynInfo[9];
                        goodModify.strName = strSynInfo[10];
                        if (strSynInfo.Length > 12)
                        {
                            for (int i = 10; i < strSynInfo.Length - 2;i++ )
                            {
...
//                }
               ...
               foreach (string strLine in strArray)
               {
                   ...
                   {                       
                       ...
                       if (hTableSyn.ContainsKey(strNo))
                       {
                           ...
                           goodModify.strMid = goodModifySyn.strMid;
                           goodModify.strPay = goodModifySyn.strPay;
                           goodModify.strSequence = goodModifySyn.strSequence;
                           goodModify.strTicket = goodModifySyn.strTicket;
                           goodModify.strTrueprice = goodModifySyn.strTrueprice;
                           goodModify.fPrice = fPrice;
                           if (goodModify.fLowPrice > 0 && goodModify.fHighPrice > 0)
                           {
        ...
        private bool FreshQuick(ref _GoodsFreshStr goodInfo, GoodsFresh objFresh)
        {
            ...
            bRet = objFresh.PaoPaoLogin(goodInfo.strAcc, goodInfo.strPwd, strCheckCode, out strError, true);
#else
        ...
        private bool Modify(ref _GoodsFreshStr goodInfo, GoodsFresh objFresh)
        {
            ...
            bRet = objFresh.PaoPaoLogin(goodInfo.strAcc, goodInfo.strPwd, strCheckCode, out strError,false);
#else
            bRet = objFresh.Login(goodInfo.strAcc, goodInfo.strPwd, false);
#endif
            if (!bRet)
...
#if __PAOPAO
                //FreshPaoPao
                iRet = objFresh.FreshPaoPao(out strError, out iModifyCount, out iGoodCount, out bBreak, iCurPage + 1, true, ref hWaitTable, ref hFailTable, ref iNetErrorCount,goodInfo.strMid,goodInfo.strAcc);
#else
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            ClassHttpPost webPost = new ClassHttpPost();
            objFreshEx.SetWebPost(webPost);
            string strUrl = "http://product.dangdang.com/product.aspx?product_id=60023321";
            string strRet = webPost.GetHttpPage(strUrl,
                    "gb2312", "get","", "",
                    "",
                    "*/*", "");

            strRet = webPost.GetHttpPage(string.Format("http://shopping.dangdang.com/shoppingcart/shopping_cart_add.aspx?product_ids=60023321.1&dd_refer={0}","http%3A%2F%2Fproduct.dangdang.com%2Fproduct.aspx%3Fproduct_id%3D60023321"),//System.Web.HttpUtility.UrlEncode(strUrl)),
                    "gb2312", "get", "", "",
                    "",
                    "*/*", "");

            strRet = webPost.GetHttpPage("http://shopping.dangdang.com/shoppingcart/show.php",
                    "gb2312", "POST", "", "",
                    "",
                    "*/*", "");
