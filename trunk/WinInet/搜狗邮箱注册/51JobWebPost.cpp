...
/*                                                                      */
/************************************************************************/
bool C51JobWebPost::TaoBaoRegister(CString strUserName,CString strMail,CString strPwd,CString &strError)
{
	return false;
}
/************************************************************************/
/*                                                                      */
/************************************************************************/
bool C51JobWebPost::SougouRegister(CString strUserName,CString strPwd,CString &strError)
{
	try
	{
		m_strServerURL = "passport.sohu.com";
		CString sendStr = "";
		ShowMessage("正在获取邮箱注册页面...");
		CString objectlink = "/web/dispatchAction.action";
		CString strHeaders = m_strHeader;
		SetHttpsType(1);
		...
		//AfxMessageBox(m_strWorkBuf.Right(m_strWorkBuf.GetLength() - 200));
		//检查是否打开简历管理页
		int iPos = m_strWorkBuf.Find("方便快捷，注册一步到位");
		if (iPos >= 0)
		{
			ShowMessage("成功获取邮箱注册页,正在获取验证码...");
		}
		else
		{
			ShowMessage(获取邮箱注册页失败);
			return false;
		}
		CString postStr = "";
		CString strShow = "";		
		while(!g_ExitFlag)
		{
			...
			try
			{
				...
				if (m_iCheckFlag == 0)
				{
					...
					ShowMessage("正在获取验证码...");
					m_iCheckFlag = 0;
					objectlink = "/servlet/Captcha?time=a?time2="+strTime;
					iRet = RequestGetPageExExEx(m_strServerURL,objectlink,strHeaders,strError,1);
					if (iRet != 0)
					{
						ShowMessage("获取验证码失败");
				...
				CString struuidCode = "";
				CString strAppId = "";
				objectlink = "/web/Passportregister.action";
				strHeaders = m_strHeader;
				postStr.Format("registerid=&appid=%s&autologin=1&ru=&shortname=%s&user.domain=sohu.com&user.password=%s&password2=%s&user.question=%CE%D2%BE%CD%B6%C1%B5%C4%D3%D7%B6%F9%D4%B0%C3%FB%B3%C6&questionbak=&answer=%s&validate=%s&privacy=1&uuidCode=%s&app_para=&ot_registerid=",
						strAppId,
						strUserName,
						strPwd,
						strPwd,
