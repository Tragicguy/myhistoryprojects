...
BEGIN_MESSAGE_MAP(CzhaochaDlg, CDialog)
		...
		ON_WM_TIMER()
		ON_BN_CLICKED(IDC_BTN_SHOWWAWA, OnBnClickedBtnShowwawa)
		ON_MESSAGE(WM_MY_MESSAGE,ButtonMove)
...
BOOL CzhaochaDlg::OnInitDialog()
{
		...
		//初始化显示信息
		showFlag = 0;
		MoveWindow(0,0,390,420);
		GetDlgItem(IDC_BTN_FIND)->MoveWindow(30,30,33,33);
// 		GetDlgItem(IDC_BTN_SHOWWAWA)->MoveWindow(350,350,33,33);
// 		oltBtnRect = CRect(350,350,383,383);
// 		m_btnshow.SubclassDlgItem(IDC_BTN_SHOWWAWA,this);
// 		m_btnshow.SetIcon(IDR_MAINFRAME);
// 		m_btnshow.DrawBorder(FALSE);
// 		m_btnshow.DrawTransparent(TRUE);

		m_btn.SubclassDlgItem(IDC_BTN_FIND,this);
		m_btn.SetIcon(IDI_ICON_LEFT);
		m_btn.SetBitmap(LoadBitmap(0,MAKEINTRESOURCE("c:\\aa.bmp")));
...
/************************************************************************/
//控制显示动态图标（与找茬本身无关）
void CzhaochaDlg::OnTimer(UINT nIDEvent)
{
		...
		if (nIDEvent == 1)
		{
				...
				{
						m_btn.SetIcon(IDI_ICON_RIGHT);
				}
				showFlag = (showFlag + 1)%2;
...
/************************************************************************/
//按钮移动（与找茬本身无关）
LRESULT CzhaochaDlg::ButtonMove(WPARAM wParam,LPARAM lParam)
{
		if(!(((CWnd*)lParam)->GetSafeHwnd() == GetDlgItem(IDC_BTN_SHOWWAWA)->GetSafeHwnd()))
		{
				return 0L;
		}
		srand(time(NULL));
		int x = 0;
		int y = 0;
		CRect rect = CRect(CPoint(63,63),CPoint(357,365));
		while(true)
		{
				x = rand() % 350;
				y = rand() % 363;
				if (IsInRect(CPoint(x,y),rect)&&!IsInRect(CPoint(x,y),oltBtnRect))
				{
						break;
				}
		}
		m_btnshow.DrawBorder(TRUE);
		GetDlgItem(IDC_BTN_SHOWWAWA)->MoveWindow(x,y,33,33);

		m_btnshow.DrawBorder(FALSE);
		m_btnshow.DrawTransparent(TRUE);
		oltBtnRect = CRect(x,y,x+33,y+33);
		return 0L;
}
...
/************************************************************************/
//运动中的按钮被点中（与找茬本身无关）
void CzhaochaDlg::OnBnClickedBtnShowwawa()
{
		MessageBox(_T("嘿嘿,坏家伙,这都被你点到我了!\r\n         来,亲一个!"),_T("你太牛了"));
		::PostMessage(AfxGetApp()->GetMainWnd()->GetSafeHwnd(),WM_MY_MESSAGE,0,(LPARAM)GetDlgItem(IDC_BTN_SHOWWAWA));
}
/************************************************************************/
/*                                                                      */
/************************************************************************/
//是否在指定获取内（与找茬本身无关）
bool CzhaochaDlg::IsInRect(CPoint point,CRect &rect)
{
		int x = point.x;
		int y = point.y;
