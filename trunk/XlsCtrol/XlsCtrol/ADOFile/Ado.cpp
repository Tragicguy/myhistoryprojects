...
/*       连接Excel数据库                                                */
/************************************************************************/
BOOL CAdoConnection::ConnectExcel(CString dbpath,long lOptions)
{
	CString strConnect = _T("Provider=Microsoft.Jet.OLEDB.4.0;Data Source=");   
	strConnect += dbpath;   //excel   file   name   
	strConnect += _T(";Extended Properties=\"Excel 8.0;HDR=Yes;IMEX=2\""); 
	return Open(LPCTSTR(strConnect), lOptions);
}
/************************************************************************/
/*通过myodbc连接mysql数据库                                              */
/************************************************************************/
BOOL CAdoConnection::ConnectMysql(CString strServer,CString strDatabase,CString strUID,CString strPwd,CString strCharset,int iPort,long lOptions)
{
	CString strConnect = "";
	if (strCharset == "")
	{
...
/*连接oracle数据库                                                       */
/************************************************************************/
BOOL CAdoConnection::ConnectToOracle(CString strServer,CString strDatabase,CString strUID,CString strPwd,long lOptions)
{
	CString strConnect = "";
...
void CAdoConnection::Close()
{
	...
	{
		TRACE(_T("Warning: 关闭数据库连接时发生未知错误:"));
	}
}
...
_RecordsetPtr CAdoConnection::Execute(LPCTSTR lpszSQL, long lOptions)
{
	...
	{
		TRACE(_T("Warning: Execute发生未知错误:"));
	}
	return FALSE;
}
	...
	try
	{
		if (m_pConnection != NULL)
		{
			...
			for (long n = 0; n < pErrors->Count; n++)
			{
				...
				strError.Format(_T("%s"),(LPCTSTR)pError->Description);
...
void CAdoConnection::OpenSchema(CStringArray &strArray)
{
	_RecordsetPtr pRst(__uuidof(Recordset));    //数据集
	pRst = OpenSchema(adSchemaTables);
	while (!pRst->adoEOF)
	{
		_bstr_t   table_name = pRst->Fields->GetItem("TABLE_NAME")->Value;//工作表名称
		CString sSheetName = (LPCTSTR)table_name;//这里的table_name就是sheet1$		
		if (sSheetName.Find("$") > 0)
		{
			sSheetName.Replace("$","");
			strArray.Add(sSheetName);
		}
		pRst->MoveNext();
	}	
}
