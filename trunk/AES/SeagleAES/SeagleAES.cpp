...
SeagleAES::SeagleAES(int keySize, byte* keyBytes)
{
	key = NULL;
	w = NULL;
...
byte** SeagleAES::AllocateArray(short iDemention0,short iDemention1)
{
	try
	{
		byte** m = (byte**)malloc(sizeof(byte *) * iDemention0); 

		for (short i = 0; i < iDemention0; i++) 
		{
			m[i] = (byte*)malloc(sizeof(byte) * iDemention1); 
		}
...
void SeagleAES::SetKeyExpansionValue(byte** KeySchedule,byte* Key,
									 short row, short index1, short index2)
{
	if(KeySchedule != NULL)
	{
		byte temp;
		memcpy(&temp,Key+index2,sizeof(byte));

		KeySchedule[row][index1] = temp;
	}
}

void SeagleAES::SetNbNkNr(int keySize)
{
...
byte* SeagleAES::SubWord(byte word[])
{
	byte* result = new byte[4];

	memcpy(result,Sbox[ word[0] >> 4, word[0] & 0x0f ],sizeof(byte));
	memcpy(result + 1,Sbox[ word[1] >> 4, word[1] & 0x0f ],sizeof(byte));
	memcpy(result + 2,Sbox[ word[2] >> 4, word[2] & 0x0f ],sizeof(byte));
	memcpy(result + 3,Sbox[ word[3] >> 4, word[3] & 0x0f ],sizeof(byte));

	return result;
}


byte* SeagleAES::RotWord(byte word[])
{
    byte* result = new byte[4];
    result[0] = word[1];
    result[1] = word[2];
...
void SeagleAES::KeyExpansion()
{
	w = AllocateArray(Nb * (Nr+1), 4);  // 4 columns of bytes corresponds to a word
	SizeofW = Nb * (Nr+1);

	for (int row = 0; row < Nk; ++row)
	{
		SetKeyExpansionValue(w,key,row,0,4*row);
		SetKeyExpansionValue(w,key,row,1,4*row + 1);
		SetKeyExpansionValue(w,key,row,2,4*row + 2);
		SetKeyExpansionValue(w,key,row,3,4*row + 3);
	}
	...
	for (int row = Nk; row < Nb * (Nr+1); ++row)
	{
		...
		if (row % Nk == 0)  
		{
			...
			byte* ptemp1 = RotWord(temp);
			memcpy(tm,ptemp1,4);
			delete ptemp1;

			byte* ptemp = SubWord(tm);
			memcpy(temp,ptemp,4);
			delete ptemp;
...
void SeagleAES::AddRoundKey(int round)
{
    for (int r = 0; r < 4; ++r)
    {
		for (int c = 0; c < 4; ++c)
...
byte SeagleAES::gfmultby01(byte b)
{
    return b;
}

byte SeagleAES::gfmultby02(byte b)
{
    if (b < 0x80)
		return (byte)(int)(b <<1);
    else
...
byte SeagleAES::gfmultby03(byte b)
{
    return (byte) ( (int)gfmultby02(b) ^ (int)b );
}

byte SeagleAES::gfmultby09(byte b)
{
    return (byte)( (int)gfmultby02(gfmultby02(gfmultby02(b))) ^
                    (int)b );
}

byte SeagleAES::gfmultby0b(byte b)
{
    return (byte)( (int)gfmultby02(gfmultby02(gfmultby02(b))) ^
                    (int)gfmultby02(b) ^
                    (int)b );
...
byte SeagleAES::gfmultby0d(byte b)
{
    return (byte)( (int)gfmultby02(gfmultby02(gfmultby02(b))) ^
...
byte SeagleAES::gfmultby0e(byte b)
{
    return (byte)( (int)gfmultby02(gfmultby02(gfmultby02(b))) ^
                    (int)gfmultby02(gfmultby02(b)) ^
                    (int)gfmultby02(b) );
...
void SeagleAES::Cipher(byte* input, byte* output)  // encipher 16-bit input
{
	// state = input
    State = AllocateArray(4,Nb);  // always [4,4]
	SizeofState = 4;
...
void SeagleAES::InvCipher(byte* input, byte* output)  // decipher 16-bit input
{
    // state = input
    State = AllocateArray(4,Nb);  // always [4,4]
    for (int i = 0; i < (4 * Nb); ++i)
