...
void CDHtmlDialogEx::OnBeforeNavigate2(LPCTSTR lpszURL, DWORD nFlags,
							   LPCTSTR lpszTargetFrameName, CByteArray& baPostedData,
							   LPCTSTR lpszHeaders, BOOL* pbCancel)
{
	//CDHtmlDialog::OnBeforeNavigate2(lpszURL,nFlags,lpszTargetFrameName,baPostedData,lpszHeaders,pbCancel);
}
...
void CDHtmlDialogEx::OnBnClickedButton2()
{
	...
	<input type="hidden" name="appActionToken" value="I7ZSQnfOOzpoRACtbW0qUbrpCv8j3D" /><input type="hidden" name="appAction" value="SIGNIN" />
	...
	<input type="hidden" name="pageId" value="ape:Y25mbGV4" />
	...
	<input type="hidden" name="openid.assoc_handle" value="ape:Y25mbGV4" />

	<input type="hidden" name="openid.return_to" value="ape:aHR0cHM6Ly93d3cuYW1hem9uLmNuL2dwL3lvdXJzdG9yZT9pZT1VVEY4JnJlZl89cGRfaXJsX2d3" />
...
// 	SetItemValue("appActionToken","I7ZSQnfOOzpoRACtbW0qUbrpCv8j3D");
// 	SetItemValue("openid.pape.max_auth_age","ape:MA==");
// 	SetItemValue("openid.ns","ape:aHR0cDovL3NwZWNzLm9wZW5pZC5uZXQvYXV0aC8yLjA=");
// 	SetItemValue("openid.ns.pape","ape:aHR0cDovL3NwZWNzLm9wZW5pZC5uZXQvZXh0ZW5zaW9ucy9wYXBlLzEuMA==");
// 	SetItemValue("pageId","I7ZSQnfOOzpoRACtbW0qUbrpCv8j3D");
// 	SetItemValue("appActionToken","I7ZSQnfOOzpoRACtbW0qUbrpCv8j3D");
// 	SetItemValue("appActionToken","I7ZSQnfOOzpoRACtbW0qUbrpCv8j3D");
	CString strValue = GetItemValue("metadata1");
...
{
	//CWindowFind find;
//	CString  aaaa= find.GetContrlWindowTextEx((HWND)71780);
...
//	IHTMLDocument2*   pDocument;
	...
	this->GetDHtmlDocument(&pDoc1);
	IHTMLDocument2 *pDocFrame2 = NULL;
	GetFrameDoc(0,pDoc1,&pDoc2);		//找到第一个FRAME标签的接口指针
	GetFrameDoc(0,pDoc2,&pDoc);
	...
	BSTR bstrValue = strText.AllocSysString();
	pBody->put_innerHTML(bstrValue);
	::SysFreeString(bstrValue);
	pBody->Release();
	pDoc->Release();
	pDoc2->Release();
...
void CDHtmlDialogEx::OnTimer(UINT nIDEvent)
{
	...
	if (nIDEvent == 2)		//此计时器用于检测是否登录成功
	{
		...
		{
			ShowMessageExEx("登录服务器成功,开始加载邮箱添加页...");
			KillTimer(2);
