...
void AddToShowList(RegisterPack rgPack)
{
	...
	{
		if (g_AccList.GetItemText(i,1) == rgPack.strAcc&&g_AccList.GetItemText(i,5) != "成功")
		{
			g_AccList.SetItemText(i,5,rgPack.strMemo);
			break;
		}		
	}
	CString strLog = "";
	strLog.Format("%s,%s,%s,%s,%s",rgPack.strAcc,rgPack.strPwd,rgPack.strMail,rgPack.strIntroMan,rgPack.strMemo);
...
{
	CString strMsg = "";
	strMsg.Format("%s,%s,%s,%s",rgPack.strAcc,rgPack.strPwd,rgPack.strMail,rgPack.strIntroMan);
	SendAddMessage(0,strMsg);
	g_strCheckCode = "";
	g_iOptedFlagEx = 100;
	while(true)		//等待验证码保存成功
	{
		if (g_ExitFlag)
		{
			return -5;
	...
	case -2:
		{
			ShowMessageExEx("添加过程中出现错误,重新开始");
			return -1;
			break;
		}
	case -3:
		{
			ShowMessageExEx("账户或密码错误,跳过此账号");
			rgPack.strMemo = "账户或密码错误";
			rgPack.iStatus = 2;
			return -3;
			break;
		}
