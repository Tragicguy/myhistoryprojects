...
void CjobpostDlg::SetTableStyle()
{
	...
	conList->InsertColumn(1,"账名",LVCFMT_LEFT,120);
	conList->InsertColumn(2,"密码",LVCFMT_LEFT,120);
	conList->InsertColumn(3,"邮箱",LVCFMT_LEFT,120);
	...
	while(true)
	{
		...
		{   
			if (pWindow.GetSafeHwnd() == NULL ||
				!::IsDialogMessage(pWindow.GetSafeHwnd(), &msg))  //加上对话框消息处理
			{ 
