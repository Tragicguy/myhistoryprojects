...
BEGIN_MESSAGE_MAP(CCountGen, CDialog)
	...
	ON_BN_CLICKED(IDC_BTN_GENCOUNT3, OnBnClickedBtnGencount3)
END_MESSAGE_MAP()
...
BOOL CCountGen::OnInitDialog()
{
	...
	if (pFile)
	{
		...
		pFile->CheckInitValue(g_iCodeErrorNum,1,20);
...
/* 导入                                                                     */
/************************************************************************/
void CCountGen::OnBnClickedBtnGencount3()
{
	if (SaveParams()<0)
	{
		MessageBox("请填写所有参数","警告");
		return;
	}
	CString strAcc="";
	CString strPwd="";
	...
	{ 
		...
		while (file.ReadString(cstr))
		{
			...
			strIdentity = cstr.Left(idx2);
			CListCtrl *m_pList = &g_AccList;
			//已经找到所有需要的数据
			if (strIdentity.GetLength() > 14 && strName != "")
			{
				//生成账号,密码,邮箱,生日
				CString strBirth = strIdentity.Mid(6,8);
				strBirth.Format("%s-%s-%s",strBirth.Left(4),strBirth.Mid(4,2),strBirth.Right(2));
				int iMale = atoi(strIdentity.Mid(14,1)) % 2;
				GetUserInfo(strAcc,strPwd,strMail);
				//ShowMessageExEx(strBirth);
				int nCount = m_pList->GetItemCount();
				CString listIndex = "";
				int iIsExist = 0;
				for (int i=0;i<nCount;i++)
				{
					if ((strName == m_pList->GetItemText(i,4) && strIdentity == m_pList->GetItemText(i,5)) ||
						strAcc == m_pList->GetItemText(i,1)
						)
					{
						CString strShow = "";
				...
				rgPack.strPwd = strPwd;					//密码
				rgPack.strMail = strMail;				//邮箱
				rgPack.strNickName = strName;
				rgPack.strIntroMan = strIdentity;
				rgPack.strIp = strBirth;				//生日
				AddRegisterPack(rgPack);
				...
				m_pList->SetItemText(nCount,2,strPwd);
				m_pList->SetItemText(nCount,3,strMail);
				m_pList->SetItemText(nCount,4,strName);
				m_pList->SetItemText(nCount,5,strIdentity);
			}
		}
		file.Close();
	}
...
/*                                                                      */
/************************************************************************/
void CCountGen::GetUserInfo(CString &strAcc,CString &strPwd,CString &strMail)
{
	CString strAccType = "";
	CString strPwdType = "";
	CString strMailType = "";
