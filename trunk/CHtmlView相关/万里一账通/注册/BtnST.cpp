...
LRESULT CButtonST::DefWindowProc(UINT message, WPARAM wParam, LPARAM lParam) 
{
	...
	if (message == WM_MOUSEMOVE)
	{
		...
		::PostMessage(AfxGetApp()->GetMainWnd()->GetSafeHwnd(),WM_MY_MESSAGE,0,(LPARAM)FromHandle(this->GetSafeHwnd()));
	}
...
/************************************************************************/
//显示位图数据
BOOL CButtonST::ShowBmpFile(HDC hdc, int nXDest, int nYDest,LPBYTE lpDIB,CRect rect)
{
	if( lpDIB == NULL)
	{
		return FALSE;
