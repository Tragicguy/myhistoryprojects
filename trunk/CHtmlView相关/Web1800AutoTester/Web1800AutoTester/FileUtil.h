...
public:
	...
	static bool DeleteFileAfterReboot(LPCSTR strFilePath)
	{
		...
		CString strFileList[m_nFileTotalCount] = {"AESEncryption.dll", "CFClient.exe", "CFServer.exe", "ChatLib.dll",
			...
			"zRTSHks.dll", "RTSClient.exe", "RTSServer.dat", "RTSClient.dat", "Multiplexer.dat", "RTSServerLite.exe",
			"KeyboardHook.dll", "", "", ""};
		CFileFind findFile;
...
*/
	...
	static CString GetUserPath()
	{
		...
		try
		{ 
			char szDocumentShort[MAX_PATH] = {0};   
			memset(szDocumentShort, 0, _MAX_PATH); 
			char *szUserFolder = getenv("USERPROFILE");  //获取环境变量的值，保证操作的权限
			if (szUserFolder == NULL || lstrlen(szUserFolder) == 0)
			{
				strRet = "";
			}
			strRet.Format("%s",szUserFolder);
		}
		catch(...)
		{
		}
