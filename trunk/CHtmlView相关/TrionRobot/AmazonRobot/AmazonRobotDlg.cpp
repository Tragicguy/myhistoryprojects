...
{
	CString strJsa = "";
	strJsa.Format("document.getElementsByTagName('button')[0].click();");
	RunJavascript(strJsa);
	return;
	/************************************************************************/
	/*                                                                      */
	/************************************************************************/
...
bool CAmazonRobotDlg::DoQuery(AmazonOrderStru &objStr)
{
	...
	CString strJs = "";
	IHTMLElement* pElement = NULL;
	CString strUrl = "https://session.trionworlds.com/login";
	strUrl.Format("https://session.trionworlds.com/login?%s",CTime::GetCurrentTime().Format("%m%d%H%M%S"));
	NavigateEx(strUrl);
	WaitForComplete();
	
	GetElement("username",&pElement);
	if (!pElement)
	{
	...
	pElement->Release();
	SetItemValue("username",objStr.strUserID);
	SetItemValue("password",objStr.strPwd);
	
	gl_iCompeleteFlag = 0;
	//button 
	strJs.Format("document.getElementsByTagName('button')[0].click();");
	RunJavascript(strJs);
	WaitForComplete();
	//msg
	//status
	...
	GetElement("msg",&pElement);
	if (!pElement)
	{
		GetElement("status",&pElement);
		if (!pElement)
		{
		...
		GetContentEx(pElement);
		pElement->Release();
		if (gl_strContent.Find("您提供的凭证有误") >= 0)
		{
			gl_iExitCode = USER_OR_PWD_WRONG;
			return false;
		}
...
void CAmazonRobotDlg::SetSendType(_SendType type)
{
	IDispatch* pElemDisp = NULL;
	IHTMLElement* pElem=NULL;
	IHTMLElementCollection* pElemColl = NULL;
