...
int CCountGen::SaveParams()
{
	...
	strNickName.Trim();
	mailtype.Trim();
	if (strAccType == "" || strPwdType == "" || strMailType == "" || strNickName == "" ||mailtype=="" )
	{
...
void CCountGen::OnBnClickedBtnImport()
{
	...
	{ 
		...
		while (file.ReadString(cstr))
		{
			...
			CString strMail = "";
			CString strIdentity = "";
			GenMailAndIdentity(strMail,strIdentity);
			...
			if (strAcc !="" && strPwd != "")
			{
				...
				{
					if (strAcc == m_pList->GetItemText(i,1))
					{
						CString strShow = "";
				...
				rgPack.strPwd = strPwd;					//密码
				rgPack.strMail = strMail;				//邮箱
				rgPack.strIdentity = strIdentity;
				rgPack.strPhone = strPhone;		//昵称
				rgPack.strIntroMan = strRecommandPhone;		//身份证号
				AddRegisterPack(rgPack);
				...
				m_pList->SetItemText(nCount,2,strPwd);
				m_pList->SetItemText(nCount,3,strMail);
				m_pList->SetItemText(nCount,4,strIdentity);
				m_pList->SetItemText(nCount,7,strPhone);
				m_pList->SetItemText(nCount,8,strRecommandPhone);
			}
		}
		file.Close();
	}
...
/*                                                                      */
/************************************************************************/
int CCountGen::StringSplit(CString srcStr,CString strArray[],char splitChar)
{
	try
	{
		int idx1 = 0;
...
/*                                                                      */
/************************************************************************/
void CCountGen::GenMailAndIdentity(CString &strMailEx,CString &strIdentityEx)
{
	CString strAccType = "";
	CString strPwdType = "";
	CString strMailType = "";
