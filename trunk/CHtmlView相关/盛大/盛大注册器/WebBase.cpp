								 ...
								 )
{
	...
	try
	{
		...
		pHttpFile->QueryInfo(HTTP_QUERY_SET_COOKIE,m_strSetCookie);	
		//pHttpFile->QueryInfo(HTTP_QUERY_COOKIE,m_strSetCookie);
...
int CWebBase::SendRequest(CHttpFile *pHttpFile,CString &strError,CString strHeader,CString strPostData,int iPostFlag)
{
	...
	try
	{
		...
		{
			char szPostData[4048] = {0};
			strncpy(szPostData,strPostData,4048);
			if (m_iPostFlag == 0)
			{
			...
			{
				bRet = pHttpFile->SendRequest(NULL,0,szPostData,strPostData.GetLength());
			}			
		}
		if( bRet == FALSE )
		{
...
DWORD CWebBase::GetDwflag()
{
	...
	else
	{
		...
		dwFlags   =   INTERNET_FLAG_KEEP_CONNECTION | INTERNET_FLAG_TRANSFER_ASCII|INTERNET_FLAG_EXISTING_CONNECT|INTERNET_FLAG_RELOAD;   
