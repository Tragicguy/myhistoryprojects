...
CWininetHook::CWininetHook(void)
{
	...
	ReplaceIATEntryForAll( "WININET.DLL", s_pfnInternetReadFileExA, &CWininetHook::_InternetReadFileExA); 
	ReplaceIATEntryForAll( "WININET.DLL", s_pfnInternetReadFileExW, &CWininetHook::_InternetReadFileExW); 
}
...
{     
	...
	if( ntStatus == STATUS_SUCCESS && (Flags & LOAD_LIBRARY_AS_DATAFILE) == 0 )  
	{
		...
		ReplaceIATEntryInImageImportTable( hDll, "WININET.DLL", s_pfnInternetReadFileExA, &CWininetHook::_InternetReadFileExA); 
		ReplaceIATEntryInImageImportTable( hDll, "WININET.DLL", s_pfnInternetReadFileExW, &CWininetHook::_InternetReadFileExW); 
	}  
	return ntStatus;  
}  
...
{  
	...
	{  
		...
		if( GetModuleFileName( ModuleHandle, tszPath, MAX_PATH-1) )  
		{  
			CString strFile = CString(tszPath);  
...
{  
	...
	RequestStr requestInfo;
	requestInfo.strObject = W2T((LPWSTR)lpszObjectName);
	requestInfo.strType = W2T((LPWSTR)lpszVerb);
	requestInfo.hRequest = hRequest;
	requestInfo.hConnect = hConnect;

	if (g_HookHelp.m_OnOpenRequest.IsEnableCallback())
	{
		g_HookHelp.m_OnOpenRequest.call(requestInfo);
	}
	return hRequest;
}  
...
{  
	...
	{
		USES_CONVERSION;
		InternetConnectInfo connInfo;
		connInfo.lHandle = hConnect;
		connInfo.strHost =  A2T(lpszServerName);
		connInfo.strPort.Format(_T("%d"),nServerPort);
		if (g_HookHelp.m_OnInternetConnect.IsEnableCallback())
		{
			g_HookHelp.m_OnInternetConnect.call(connInfo);
		}
	}
	return hConnect;
}  
...
{  
	...
	if( hConnect )
	{
		...
		InternetConnectInfo connInfo;
		connInfo.lHandle = hConnect;
		connInfo.strHost =  W2T((LPWSTR)lpszServerName);
		connInfo.strPort.Format(_T("%d"),nServerPort);
		if (g_HookHelp.m_OnInternetConnect.IsEnableCallback())
		{
			g_HookHelp.m_OnInternetConnect.call(connInfo);
...
//	CMainFrame::GetRequestDlg().LogHttpResponseHeader(hInternet);
	...
	if (g_HookHelp.m_OnCloseHandle.IsEnableCallback())
	{
		g_HookHelp.m_OnCloseHandle.call(hInternet);
	}
...
BOOL WINAPI CWininetHook::_InternetReadFile(__in HINTERNET hFile,
									 ...
									 __out LPDWORD lpdwNumberOfBytesRead)
{
	...
	//::MessageBoxA(NULL,(char*)lpBuffer,"",NULL);
	if (g_HookHelp.m_OnNewReceived.IsEnableCallback())
	{
		BufferStr newBuffer;
		newBuffer.lHandle = hFile;
		newBuffer.pBuffer = lpBuffer;
		g_HookHelp.m_OnNewReceived.call(newBuffer);
	}
	return bRet;
}
BOOL WINAPI CWininetHook::_InternetReadFileExW(__in HINTERNET hRequest,__out LPINTERNET_BUFFERSW lpBuffersOut,__in DWORD dwFlags,__in_opt DWORD_PTR dwContent)
{
	return s_pfnInternetReadFileExW(hRequest, lpBuffersOut, dwFlags, dwContent);
}
BOOL WINAPI CWininetHook::_InternetReadFileExA(__in HINTERNET hRequest,__out LPINTERNET_BUFFERSA lpBuffersOut,__in DWORD dwFlags,__in_opt DWORD_PTR dwContent)
{
	return s_pfnInternetReadFileExA(hRequest, lpBuffersOut, dwFlags, dwContent);
}
