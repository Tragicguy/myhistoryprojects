...
int CASCIIConver::fstr2int(CString strIn)
{
	...
	for (int i=1; i >= 0; i--)
	{
		...
		iRet += (int) pow((double)16, (int)i) * iDec;		
	}
	return iRet;
}
...
int CASCIIConver::SplitMsg(CString &srcMsg, int nSplitLen, CString &strOutMsg)
{
	...
	try
	{
		...
		{
			wcsncpy_s(tempRet, temp, nSplitLen);
		...
		else if (len > nSplitLen)	//超过一条信息
		{
			...
			wcsncpy_s(tempRet,temp + nSplitLen,len - nSplitLen);
